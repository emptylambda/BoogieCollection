...
const unique class.CachedClient.__default: ClassName;

function Tclass.CachedClient.__default() : Ty;

const unique Tagclass.CachedClient.__default: TyTag;

// Tclass.CachedClient.__default Tag
axiom Tag(Tclass.CachedClient.__default()) == Tagclass.CachedClient.__default
   && TagFamily(Tclass.CachedClient.__default()) == tytagFamily$_default;

// Box/unbox axiom for Tclass.CachedClient.__default
axiom (forall bx: Box :: 
  { $IsBox(bx, Tclass.CachedClient.__default()) } 
  $IsBox(bx, Tclass.CachedClient.__default())
     ==> $Box($Unbox(bx): ref) == bx
       && $Is($Unbox(bx): ref, Tclass.CachedClient.__default()));

// _default: Class $Is
axiom (forall $o: ref :: 
  { $Is($o, Tclass.CachedClient.__default()) } 
  $Is($o, Tclass.CachedClient.__default())
     <==> $o == null || dtype($o) == Tclass.CachedClient.__default());

// _default: Class $IsAlloc
axiom (forall $o: ref, $h: Heap :: 
  { $IsAlloc($o, Tclass.CachedClient.__default(), $h) } 
  $IsAlloc($o, Tclass.CachedClient.__default(), $h)
     <==> $o == null || read($h, $o, alloc));

procedure CheckWellformed$$CachedClient.__default._default_Main();
  free requires 2 == $FunctionContextHeight;
  modifies $Heap, $Tick;



procedure Call$$CachedClient.__default._default_Main();
  modifies $Heap, $Tick;
...
procedure Impl$$CachedClient.__default._default_Main() returns ($_reverifyPost: bool);
  free requires 2 == $FunctionContextHeight;
  modifies $Heap, $Tick;
  // frame condition: object granularity
  free ensures (forall $o: ref :: 
...
implementation Impl$$CachedClient.__default._default_Main() returns ($_reverifyPost: bool)
{
  var $_Frame: <beta>[ref,Field beta]bool;

    // AddMethodImpl: Main, Impl$$CachedClient.__default._default_Main
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny3/CachedContainer.dfy(215,16): initial state"} true;
    $_reverifyPost := false;
    // ----- call statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny3/CachedContainer.dfy(216,9)
    // TrCallStmt: Before ProcessCallStmt
    assert (forall<alpha> $o: ref, $f: Field alpha :: false ==> $_Frame[$o, $f]);
    // ProcessCallStmt: Make the call
    call Call$$CachedClient.__default.Test();
    // TrCallStmt: After ProcessCallStmt
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny3/CachedContainer.dfy(216,10)"} true;
}



procedure CheckWellformed$$CachedClient.__default.Test();
  free requires 1 == $FunctionContextHeight;
  modifies $Heap, $Tick;



procedure Call$$CachedClient.__default.Test();
  modifies $Heap, $Tick;
  // frame condition: object granularity
  free ensures (forall $o: ref :: 
    { $Heap[$o] } 
...
procedure Impl$$CachedClient.__default.Test() returns ($_reverifyPost: bool);
  free requires 1 == $FunctionContextHeight;
  modifies $Heap, $Tick;
  // frame condition: object granularity
  free ensures (forall $o: ref :: 
...
axiom (forall a#5#0#0: int, a#5#1#0: Box :: 
  { #M3.Cache.Some(a#5#0#0, a#5#1#0) } 
  DatatypeCtorId(#M3.Cache.Some(a#5#0#0, a#5#1#0)) == ##M3.Cache.Some);
...
axiom (forall d: DatatypeType :: 
  { M3.Cache.Some_q(d) } 
  M3.Cache.Some_q(d)
     ==> (exists a#6#0#0: int, a#6#1#0: Box :: d == #M3.Cache.Some(a#6#0#0, a#6#1#0)));

// Constructor $Is
axiom (forall M3.Cache$T: Ty, a#7#0#0: int, a#7#1#0: Box :: 
  { $Is(#M3.Cache.Some(a#7#0#0, a#7#1#0), Tclass.M3.Cache(M3.Cache$T)) } 
  $Is(#M3.Cache.Some(a#7#0#0, a#7#1#0), Tclass.M3.Cache(M3.Cache$T))
     <==> $Is(a#7#0#0, Tclass._System.nat()) && $IsBox(a#7#1#0, M3.Cache$T));

// Constructor $IsAlloc
axiom (forall M3.Cache$T: Ty, a#8#0#0: int, a#8#1#0: Box, $h: Heap :: 
  { $IsAlloc(#M3.Cache.Some(a#8#0#0, a#8#1#0), Tclass.M3.Cache(M3.Cache$T), $h) } 
  $IsGoodHeap($h)
     ==> ($IsAlloc(#M3.Cache.Some(a#8#0#0, a#8#1#0), Tclass.M3.Cache(M3.Cache$T), $h)
       <==> $IsAlloc(a#8#0#0, Tclass._System.nat(), $h)
         && $IsAllocBox(a#8#1#0, M3.Cache$T, $h)));
...
axiom (forall a#9#0#0: int, a#9#1#0: Box :: 
  { #M3.Cache.Some(LitInt(a#9#0#0), Lit(a#9#1#0)) } 
  #M3.Cache.Some(LitInt(a#9#0#0), Lit(a#9#1#0))
     == Lit(#M3.Cache.Some(a#9#0#0, a#9#1#0)));

function M3.Cache.index(DatatypeType) : int;

// Constructor injectivity
axiom (forall a#10#0#0: int, a#10#1#0: Box :: 
  { #M3.Cache.Some(a#10#0#0, a#10#1#0) } 
  M3.Cache.index(#M3.Cache.Some(a#10#0#0, a#10#1#0)) == a#10#0#0);

function M3.Cache.value(DatatypeType) : Box;

// Constructor injectivity
axiom (forall a#11#0#0: int, a#11#1#0: Box :: 
  { #M3.Cache.Some(a#11#0#0, a#11#1#0) } 
  M3.Cache.value(#M3.Cache.Some(a#11#0#0, a#11#1#0)) == a#11#1#0);

// Inductive rank
axiom (forall a#12#0#0: int, a#12#1#0: Box :: 
  { #M3.Cache.Some(a#12#0#0, a#12#1#0) } 
  BoxRank(a#12#1#0) < DtRank(#M3.Cache.Some(a#12#0#0, a#12#1#0)));
       ...
       || (M3.Container.Valid_k#canCall(M3.Container$T, $Heap, this)
         ==> M3.Container.Valid_k(M3.Container$T, $Heap, this)
           || Set#Equal(read($Heap, this, M3.Container.Contents), 
            (lambda $y#10: Box :: 
              $IsBox($y#10, M3.Container$T)
                 && Seq#Contains(read($Heap, this, M3.Container.elems), $y#10))));
  requires M3.Container.Valid#canCall(M3.Container$T, $Heap, this)
     ==> M3.Container.Valid(M3.Container$T, $Heap, this)
       || (M3.Container.Valid_k#canCall(M3.Container$T, $Heap, this)
         ==> M3.Container.Valid_k(M3.Container$T, $Heap, this)
           || (forall i#5: int, j#6: int :: 
            { Seq#Index(read($Heap, this, M3.Container.elems), j#6), Seq#Index(read($Heap, this, M3.Container.elems), i#5) } 
            true
               ==> 
              LitInt(0) <= i#5
                 && i#5 < j#6
                 && j#6 < Seq#Length(read($Heap, this, M3.Container.elems))
               ==> Seq#Index(read($Heap, this, M3.Container.elems), i#5)
                 != Seq#Index(read($Heap, this, M3.Container.elems), j#6)));
  requires M3.Container.Valid#canCall(M3.Container$T, $Heap, this)
     ==> M3.Container.Valid(M3.Container$T, $Heap, this)
       || (M3.Container.Valid_k#canCall(M3.Container$T, $Heap, this)
         ==> M3.Container.Valid_k(M3.Container$T, $Heap, this)
...
axiom true
   ==> (forall M3.Container$T: Ty, $Heap: Heap, this: ref :: 
    ...
    M3.Container.Valid_k#canCall(M3.Container$T, $Heap, this)
       ...
       ==> (Set#Equal(read($Heap, this, M3.Container.Contents), 
            (lambda $y#7: Box :: 
              $IsBox($y#7, M3.Container$T)
                 && Seq#Contains(read($Heap, this, M3.Container.elems), $y#7)))
           ==> 
     ...
     ==> M3.Container.Valid(M3.Container$T, $Heap, this)
       || (M3.Container.Valid_k#canCall(M3.Container$T, $Heap, this)
         ==> M3.Container.Valid_k(M3.Container$T, $Heap, this)
           || Set#Equal(read($Heap, this, M3.Container.Contents), 
            (lambda $y#8: Box :: 
              $IsBox($y#8, M3.Container$T)
                 && Seq#Contains(read($Heap, this, M3.Container.elems), $y#8))));
  requires M3.Container.Valid#canCall(M3.Container$T, $Heap, this)
     ==> M3.Container.Valid(M3.Container$T, $Heap, this)
       || (M3.Container.Valid_k#canCall(M3.Container$T, $Heap, this)
         ==> M3.Container.Valid_k(M3.Container$T, $Heap, this)
           || (forall i#4: int, j#5: int :: 
            { Seq#Index(read($Heap, this, M3.Container.elems), j#5), Seq#Index(read($Heap, this, M3.Container.elems), i#4) } 
            true
               ==> 
              LitInt(0) <= i#4
                 && i#4 < j#5
                 && j#5 < Seq#Length(read($Heap, this, M3.Container.elems))
               ==> Seq#Index(read($Heap, this, M3.Container.elems), i#4)
                 != Seq#Index(read($Heap, this, M3.Container.elems), j#5)));
  requires M3.Container.Valid#canCall(M3.Container$T, $Heap, this)
     ==> M3.Container.Valid(M3.Container$T, $Heap, this)
       || (M3.Container.Valid_k#canCall(M3.Container$T, $Heap, this)
         ==> M3.Container.Valid_k(M3.Container$T, $Heap, this)
