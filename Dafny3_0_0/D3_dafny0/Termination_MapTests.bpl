...
axiom 1 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, a#0: Map Box Box, n#0: int :: 
    ...
    MapTests.__default.F#canCall(a#0, n#0)
         || (1 != $FunctionContextHeight && $Is(a#0, TMap(TInt, TInt)) && LitInt(0) <= n#0)
       ==> true);
...
procedure CheckWellformed$$MapTests.__default.F(a#0: Map Box Box where $Is(a#0, TMap(TInt, TInt)), 
...
implementation CheckWellformed$$MapTests.__default.F(a#0: Map Box Box, n#0: int)
{
  ...
  var b$reqreads#0: bool;
    ...
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Termination.dfy(402,11): initial state"} true;
    ...
    else
    {
        ...
        if (n#0 == LitInt(0))
        {
            assume MapTests.__default.F($LS($LZ), a#0, n#0) == LitInt(0);
            assume true;
            // CheckWellformedWithResult: any expression
            assume $Is(MapTests.__default.F($LS($LZ), a#0, n#0), TInt);
        }
        else
        {
            ##a#0 := a#0;
            // assume allocatedness for argument to function
            assume $IsAlloc(##a#0, TMap(TInt, TInt), $Heap);
            ...
            assert 0 <= n#0
               || (Set#Subset(Map#Domain(##a#0), Map#Domain(a#0))
                 && !Set#Subset(Map#Domain(a#0), Map#Domain(##a#0)))
               || ##n#0 == n#0;
            assert (Set#Subset(Map#Domain(##a#0), Map#Domain(a#0))
                 && !Set#Subset(Map#Domain(a#0), Map#Domain(##a#0)))
               || (Set#Equal(Map#Domain(##a#0), Map#Domain(a#0)) && ##n#0 < n#0);
            assume MapTests.__default.F#canCall(a#0, n#0 - 1);
            assume MapTests.__default.F($LS($LZ), a#0, n#0)
               == MapTests.__default.F($LS($LZ), a#0, n#0 - 1);
            assume MapTests.__default.F#canCall(a#0, n#0 - 1);
            // CheckWellformedWithResult: any expression
            assume $Is(MapTests.__default.F($LS($LZ), a#0, n#0), TInt);
        }
...
axiom 2 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, a#0: Map Box Box, n#0: int :: 
    ...
    MapTests.__default.F_k#canCall(a#0, n#0)
         || (2 != $FunctionContextHeight && $Is(a#0, TMap(TInt, TInt)) && LitInt(0) <= n#0)
       ==> true);
...
procedure CheckWellformed$$MapTests.__default.F_k(a#0: Map Box Box where $Is(a#0, TMap(TInt, TInt)), 
    n#0: int where LitInt(0) <= n#0);
...
implementation CheckWellformed$$MapTests.__default.F_k(a#0: Map Box Box, n#0: int)
{
  var $_Frame: <beta>[ref,Field beta]bool;
  var ##a#0: Map Box Box;
  var ##n#0: int;
  var b$reqreads#0: bool;
    ...
    // AddWellformednessCheck for function F'
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Termination.dfy(408,11): initial state"} true;
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    if (*)
    {
    ...
    else
    {
        ...
        if (n#0 == LitInt(0))
        {
            assume MapTests.__default.F_k($LS($LZ), a#0, n#0) == LitInt(0);
            assume true;
            // CheckWellformedWithResult: any expression
            assume $Is(MapTests.__default.F_k($LS($LZ), a#0, n#0), TInt);
        }
        else
        {
            ##a#0 := a#0;
            ...
            assert (Set#Subset(Map#Domain(##a#0), Map#Domain(a#0))
                 && !Set#Subset(Map#Domain(a#0), Map#Domain(##a#0)))
               || (Set#Equal(Map#Domain(##a#0), Map#Domain(a#0)) && ##n#0 < n#0);
            assume MapTests.__default.F_k#canCall(a#0, n#0 - 1);
            assume MapTests.__default.F_k($LS($LZ), a#0, n#0)
               == MapTests.__default.F_k($LS($LZ), a#0, n#0 - 1);
            assume MapTests.__default.F_k#canCall(a#0, n#0 - 1);
            // CheckWellformedWithResult: any expression
            assume $Is(MapTests.__default.F_k($LS($LZ), a#0, n#0), TInt);
        }
...
procedure {:_induction n#0, b#0} CheckWellformed$$MapTests.__default.M(n#0: int where LitInt(0) <= n#0, 
    b#0: Map Box Box
...
procedure {:_induction n#0, b#0} Call$$MapTests.__default.M(n#0: int where LitInt(0) <= n#0, 
    b#0: Map Box Box
       where $Is(b#0, TMap(TInt, TInt)) && $IsAlloc(b#0, TMap(TInt, TInt), $Heap));
  modifies $Heap, $Tick;
  // user-defined postconditions
  free ensures MapTests.__default.F#canCall(b#0, n#0);
  ensures MapTests.__default.F($LS($LS($LZ)), b#0, n#0) == LitInt(0);
  // frame condition
  free ensures old($Heap) == $Heap;



procedure {:_induction n#0, b#0} Impl$$MapTests.__default.M(n#0: int where LitInt(0) <= n#0, 
    b#0: Map Box Box
       where $Is(b#0, TMap(TInt, TInt)) && $IsAlloc(b#0, TMap(TInt, TInt), $Heap))
...
implementation {:_induction n#0, b#0} Impl$$MapTests.__default.M(n#0: int, b#0: Map Box Box) returns ($_reverifyPost: bool)
{
  var $_Frame: <beta>[ref,Field beta]bool;
  var $initHeapForallStmt#0: Heap;

    // AddMethodImpl: M, Impl$$MapTests.__default.M
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Termination.dfy(415,2): initial state"} true;
    $initHeapForallStmt#0 := $Heap;
    havoc $Heap, $Tick;
    assume $initHeapForallStmt#0 == $Heap;
    assume (forall $ih#n0#0: int, $ih#b0#0: Map Box Box :: 
      LitInt(0) <= $ih#n0#0
           && $Is($ih#b0#0, TMap(TInt, TInt))
           && Lit(true)
           && ((0 <= $ih#n0#0 && $ih#n0#0 < n#0)
             || ($ih#n0#0 == n#0
               && 
              Set#Subset(Map#Domain($ih#b0#0), Map#Domain(b#0))
               && !Set#Subset(Map#Domain(b#0), Map#Domain($ih#b0#0))))
         ==> MapTests.__default.F($LS($LZ), $ih#b0#0, $ih#n0#0) == LitInt(0));
    $_reverifyPost := false;
}
