...
axiom 0 <= $FunctionContextHeight
   ==> 
  LetSuchThat.__default.Bad#canCall() || 0 != $FunctionContextHeight
   ==> true;
...
axiom 0 <= $FunctionContextHeight
   ==> (forall $Heap: Heap :: 
    ...
    LetSuchThat.__default.Bad#canCall()
...
procedure CheckWellformed$$LetSuchThat.__default.Bad();
  free requires 0 == $FunctionContextHeight;
  modifies $Heap, $Tick;



implementation CheckWellformed$$LetSuchThat.__default.Bad()
{
  ...
  var c#1: Box;
    ...
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/DefiniteAssignment.dfy(348,11): initial state"} true;
    ...
    else
    {
        ...
        assert (exists c#0: Box :: $IsBox(c#0, #$C) && Lit(true));
        assume $IsBox(c#1, #$C);
        assume true;
        assume $let#0$canCall();
        assume LetSuchThat.__default.Bad() == LitInt(5);
        assume true;
        // CheckWellformedWithResult: Let expression
        assume $Is(LetSuchThat.__default.Bad(), TInt);
    }
}
...
axiom 1 <= $FunctionContextHeight
   ==> (forall y#0: Box :: 
    ...
    LetSuchThat.__default.Good#canCall(y#0)
         || (1 != $FunctionContextHeight && $IsBox(y#0, #$C))
       ==> true);
...
axiom 1 <= $FunctionContextHeight
   ==> (forall $Heap: Heap, y#0: Box :: 
    ...
    LetSuchThat.__default.Good#canCall(y#0)
         || (1 != $FunctionContextHeight && $IsGoodHeap($Heap) && $IsBox(y#0, #$C))
       ==> $let#1$canCall()
...
procedure CheckWellformed$$LetSuchThat.__default.Good(y#0: Box where $IsBox(y#0, #$C));
  free requires 1 == $FunctionContextHeight;
  modifies $Heap, $Tick;



implementation CheckWellformed$$LetSuchThat.__default.Good(y#0: Box)
{
  var $_Frame: <beta>[ref,Field beta]bool;
  var c#1: Box;
    ...
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/DefiniteAssignment.dfy(355,11): initial state"} true;
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    if (*)
    {
    ...
    else
    {
        ...
        assume LetSuchThat.__default.Good(y#0) == LitInt(5);
        assume true;
        // CheckWellformedWithResult: Let expression
        assume $Is(LetSuchThat.__default.Good(y#0), TInt);
    }
}
