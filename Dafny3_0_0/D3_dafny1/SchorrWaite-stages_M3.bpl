...
procedure CheckWellformed$$M3.__default.SchorrWaite(root#0: ref
...
procedure Call$$M3.__default.SchorrWaite(root#0: ref
       where $Is(root#0, Tclass.M3.Node()) && $IsAlloc(root#0, Tclass.M3.Node(), $Heap), 
    S#0: Set Box
       where $Is(S#0, TSet(Tclass.M3.Node())) && $IsAlloc(S#0, TSet(Tclass.M3.Node()), $Heap));
  ...
  requires (forall n#1: ref :: 
    { read($Heap, n#1, M3.Node.children) } { S#0[$Box(n#1)] } 
    $Is(n#1, Tclass.M3.Node())
       ==> 
      S#0[$Box(n#1)]
       ==> (forall ch#2: ref :: 
        { S#0[$Box(ch#2)] } 
          { Seq#Contains(read($Heap, n#1, M3.Node.children), $Box(ch#2)) } 
        $Is(ch#2, Tclass.M3.Node?())
           ==> 
          Seq#Contains(read($Heap, n#1, M3.Node.children), $Box(ch#2))
           ==> ch#2 == null || S#0[$Box(ch#2)]));
  requires (forall n#3: ref :: 
    { read($Heap, n#3, M3.Node.childrenVisited) } 
      { read($Heap, n#3, M3.Node.marked) } 
      { S#0[$Box(n#3)] } 
    $Is(n#3, Tclass.M3.Node())
       ==> (S#0[$Box(n#3)] ==> !read($Heap, n#3, M3.Node.marked))
         && (S#0[$Box(n#3)] ==> read($Heap, n#3, M3.Node.childrenVisited) == LitInt(0)));
  modifies $Heap, $Tick;
  // user-defined postconditions
  free ensures true;
  ensures read($Heap, root#0, M3.Node.marked);
  free ensures true;
  ensures (forall n#5: ref :: 
    { read($Heap, n#5, M3.Node.children) } 
      { read($Heap, n#5, M3.Node.marked) } 
      { S#0[$Box(n#5)] } 
    $Is(n#5, Tclass.M3.Node())
       ==> 
      S#0[$Box(n#5)] && read($Heap, n#5, M3.Node.marked)
       ==> (forall ch#5: ref :: 
        { read($Heap, ch#5, M3.Node.marked) } 
          { Seq#Contains(read($Heap, n#5, M3.Node.children), $Box(ch#5)) } 
        $Is(ch#5, Tclass.M3.Node?())
           ==> 
          Seq#Contains(read($Heap, n#5, M3.Node.children), $Box(ch#5)) && ch#5 != null
           ==> read($Heap, ch#5, M3.Node.marked)));
  free ensures (forall n#7: ref :: 
    { M3.__default.Reachable(old($Heap), root#0, n#7, S#0) } 
      { read($Heap, n#7, M3.Node.marked) } 
      { S#0[$Box(n#7)] } 
  ...
  ensures (forall n#7: ref :: 
    ...
    $Is(n#7, Tclass.M3.Node())
       ==> 
      S#0[$Box(n#7)] && read($Heap, n#7, M3.Node.marked)
       ==> M3.__default.Reachable(old($Heap), root#0, n#7, S#0));
  free ensures true;
  ensures (forall n#9: ref :: 
    { read(old($Heap), n#9, M3.Node.children) } 
      { read($Heap, n#9, M3.Node.children) } 
      { read(old($Heap), n#9, M3.Node.childrenVisited) } 
      { read($Heap, n#9, M3.Node.childrenVisited) } 
      { S#0[$Box(n#9)] } 
    $Is(n#9, Tclass.M3.Node())
       ==> (S#0[$Box(n#9)]
           ==> read($Heap, n#9, M3.Node.childrenVisited)
             == read(old($Heap), n#9, M3.Node.childrenVisited))
         && (S#0[$Box(n#9)]
           ==> Seq#Equal(read($Heap, n#9, M3.Node.children), read(old($Heap), n#9, M3.Node.children))));
...
procedure Impl$$M3.__default.SchorrWaite(root#0: ref
       where $Is(root#0, Tclass.M3.Node()) && $IsAlloc(root#0, Tclass.M3.Node(), $Heap), 
    S#0: Set Box
       where $Is(S#0, TSet(Tclass.M3.Node())) && $IsAlloc(S#0, TSet(Tclass.M3.Node()), $Heap))
  ...
  // user-defined postconditions
  free ensures true;
  ensures $_reverifyPost ==> read($Heap, root#0, M3.Node.marked);
  free ensures true;
...
implementation Impl$$M3.__default.SchorrWaite(root#0: ref, S#0: Set Box) returns ($_reverifyPost: bool)
{
  ...
  var $rhs#0: DatatypeType where $Is($rhs#0, Tclass.M3.Path());
  var $rhs#1: bool;
  var t#0: ref
     where $Is(t#0, Tclass.M3.Node()) && $IsAlloc(t#0, Tclass.M3.Node(), $Heap);
  var p#0: ref
     where $Is(p#0, Tclass.M3.Node?()) && $IsAlloc(p#0, Tclass.M3.Node?(), $Heap);
  var $rhs#2: ref where $Is($rhs#2, Tclass.M3.Node());
  var $rhs#3: ref where $Is($rhs#3, Tclass.M3.Node?());
  ...
  var $obj2: ref;
  var $rhs#0_0_1: ref where $Is($rhs#0_0_1, Tclass.M3.Node());
  var $rhs#0_0_2: ref where $Is($rhs#0_0_2, Tclass.M3.Node?());
  var $rhs#0_0_3: Seq Box where $Is($rhs#0_0_3, TSeq(Tclass.M3.Node?()));
  var $rhs#0_0_4: int where LitInt(0) <= $rhs#0_0_4;
  var $rhs#0_1_0: int where LitInt(0) <= $rhs#0_1_0;
  var $rhs#0_0: ref where $Is($rhs#0_0, Tclass.M3.Node());
  var $rhs#0_1: ref where $Is($rhs#0_1, Tclass.M3.Node?());
  var $rhs#0_2: Seq Box where $Is($rhs#0_2, TSeq(Tclass.M3.Node?()));
  var $rhs#0_3: bool;
  var $rhs#0_4: DatatypeType where $Is($rhs#0_4, Tclass.M3.Path());
  var n#28: ref;
  var ch#8: ref;
  var n#30: ref;
  var ##source#4: ref;
  ...
  var ##S#4: Set Box;
    ...
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy(250,2): initial state"} true;
    $_reverifyPost := false;
    // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy(255,29)
    assume true;
    assume true;
    unmarkedNodes#0 := Set#Difference(S#0, Set#UnionOne(Set#Empty(): Set Box, $Box(root#0)));
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy(255,41)"} true;
    // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M3](200,20)
    assume true;
    assume true;
    path#0 := Lit(#M3.Path.Empty());
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M3](200,32)"} true;
    // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M3](201,23)
    assume root#0 != null;
    assume true;
    assume $_Frame[root#0, M3.Node.pathFromRoot];
    assume true;
    $rhs#0 := path#0;
    $Heap := update($Heap, root#0, M3.Node.pathFromRoot, $rhs#0);
    assume $IsGoodHeap($Heap);
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M3](201,29)"} true;
    // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](57,17)
    assume root#0 != null;
    assume true;
    assume $_Frame[root#0, M3.Node.marked];
    assume true;
    $rhs#1 := Lit(true);
    $Heap := update($Heap, root#0, M3.Node.marked, $rhs#1);
    assume $IsGoodHeap($Heap);
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](57,23)"} true;
    // ----- update statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](58,21)
    assume true;
    assume true;
    assume true;
    $rhs#2 := root#0;
    ...
    t#0 := $rhs#2;
    p#0 := $rhs#3;
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](58,33)"} true;
    // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](59,37)
    assume true;
    assume true;
    stackNodes#0 := Lit(Seq#Empty(): Seq Box);
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](59,41)"} true;
    $Heap_at_0 := $Heap;
    // ----- while statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy(257,5)
    // Assume Fuel Constant
    $PreLoopHeap$loop#0 := $Heap;
    $decr_init$loop#00 := unmarkedNodes#0;
    $decr_init$loop#01 := stackNodes#0;
    $decr_init$loop#02 := Seq#Length(read($Heap, t#0, M3.Node.children))
       - read($Heap, t#0, M3.Node.childrenVisited);
    havoc $w$loop#0;
    while (true)
      ...
      free invariant $w$loop#0
         ==> (forall n#11: ref :: 
          { read($Heap, n#11, M3.Node.children) } 
            { read($Heap, n#11, M3.Node.childrenVisited) } 
            { Seq#Contains(stackNodes#0, $Box(n#11)) } 
          $Is(n#11, Tclass.M3.Node())
             ==> 
            Seq#Contains(stackNodes#0, $Box(n#11))
             ==> read($Heap, n#11, M3.Node.childrenVisited)
               < Seq#Length(read($Heap, n#11, M3.Node.children)));
      free invariant $w$loop#0 ==> true;
      free invariant $w$loop#0
         ==> read($Heap, t#0, M3.Node.childrenVisited)
           <= Seq#Length(read($Heap, t#0, M3.Node.children));
      free invariant $w$loop#0 ==> true;
      free invariant $w$loop#0
         ==> (forall n#13: ref :: 
          { read($Heap, n#13, M3.Node.childrenVisited) } 
            { Seq#Contains(stackNodes#0, $Box(n#13)) } 
            { S#0[$Box(n#13)] } 
          $Is(n#13, Tclass.M3.Node())
             ==> 
            S#0[$Box(n#13)] && !Seq#Contains(stackNodes#0, $Box(n#13)) && n#13 != t#0
             ==> read($Heap, n#13, M3.Node.childrenVisited) == LitInt(0));
      free invariant $w$loop#0 ==> true;
      free invariant $w$loop#0
         ==> (forall n#15: ref :: 
          { read($Heap, n#15, M3.Node.childrenVisited) } 
            { read(old($Heap), n#15, M3.Node.children) } 
            { read($Heap, n#15, M3.Node.children) } 
            { Seq#Contains(stackNodes#0, $Box(n#15)) } 
          $Is(n#15, Tclass.M3.Node())
             ==> (Seq#Contains(stackNodes#0, $Box(n#15))
                 ==> Seq#Length(read($Heap, n#15, M3.Node.children))
                   == Seq#Length(read(old($Heap), n#15, M3.Node.children)))
               && (Seq#Contains(stackNodes#0, $Box(n#15))
                 ==> (forall j#1: int :: 
                  { $Unbox(Seq#Index(read(old($Heap), n#15, M3.Node.children), j#1)): ref } 
                    { $Unbox(Seq#Index(read($Heap, n#15, M3.Node.children), j#1)): ref } 
                  true
                     ==> 
                    LitInt(0) <= j#1 && j#1 < Seq#Length(read($Heap, n#15, M3.Node.children))
                     ==> j#1 == read($Heap, n#15, M3.Node.childrenVisited)
                       || $Unbox(Seq#Index(read($Heap, n#15, M3.Node.children), j#1)): ref
                         == $Unbox(Seq#Index(read(old($Heap), n#15, M3.Node.children), j#1)): ref)));
      free invariant $w$loop#0 ==> true;
      free invariant $w$loop#0
         ==> (forall n#17: ref :: 
          { read(old($Heap), n#17, M3.Node.children) } 
            { read($Heap, n#17, M3.Node.children) } 
            { Seq#Contains(stackNodes#0, $Box(n#17)) } 
            { S#0[$Box(n#17)] } 
          $Is(n#17, Tclass.M3.Node())
             ==> 
            S#0[$Box(n#17)] && !Seq#Contains(stackNodes#0, $Box(n#17))
             ==> Seq#Equal(read($Heap, n#17, M3.Node.children), read(old($Heap), n#17, M3.Node.children)));
      free invariant $w$loop#0 ==> true;
      free invariant $w$loop#0
         ==> 
        0 < Seq#Length(stackNodes#0)
         ==> $Unbox(Seq#Index(read($Heap, $Unbox(Seq#Index(stackNodes#0, LitInt(0))): ref, M3.Node.children), 
              read($Heap, $Unbox(Seq#Index(stackNodes#0, LitInt(0))): ref, M3.Node.childrenVisited))): ref
           == null;
      free invariant $w$loop#0 ==> true;
      free invariant $w$loop#0
         ==> (forall k#1: int :: 
          {:matchinglooprewrite false} { read($Heap, $Unbox(Seq#Index(stackNodes#0, k#1)): ref, M3.Node.childrenVisited) } 
            { read($Heap, $Unbox(Seq#Index(stackNodes#0, k#1)): ref, M3.Node.children) } 
          true
             ==> 
            0 < k#1 && k#1 < Seq#Length(stackNodes#0)
             ==> $Unbox(Seq#Index(read($Heap, $Unbox(Seq#Index(stackNodes#0, k#1)): ref, M3.Node.children), 
                  read($Heap, $Unbox(Seq#Index(stackNodes#0, k#1)): ref, M3.Node.childrenVisited))): ref
               == $Unbox(Seq#Index(stackNodes#0, k#1 - 1)): ref);
      free invariant $w$loop#0 ==> true;
      free invariant $w$loop#0
         ==> (forall k#3: int :: 
          {:matchinglooprewrite false} { read($Heap, $Unbox(Seq#Index(stackNodes#0, k#3)): ref, M3.Node.childrenVisited) } 
            { $Unbox(Seq#Index(stackNodes#0, k#3)): ref } 
          true
             ==> 
            LitInt(0) <= k#3 && k#3 < Seq#Length(stackNodes#0) - 1
             ==> $Unbox(Seq#Index(read(old($Heap), $Unbox(Seq#Index(stackNodes#0, k#3)): ref, M3.Node.children), 
                  read($Heap, $Unbox(Seq#Index(stackNodes#0, k#3)): ref, M3.Node.childrenVisited))): ref
      ...
      free invariant $w$loop#0
        ...
        0 < Seq#Length(stackNodes#0)
         ==> $Unbox(Seq#Index(read(old($Heap), 
              ...
              read($Heap, 
                $Unbox(Seq#Index(stackNodes#0, Seq#Length(stackNodes#0) - 1)): ref, 
                M3.Node.childrenVisited))): ref
           == t#0;
      free invariant $w$loop#0 ==> true;
      free invariant $w$loop#0 ==> read($Heap, root#0, M3.Node.marked);
      free invariant $w$loop#0 ==> true;
      free invariant $w$loop#0
         ==> (forall n#19: ref :: 
          { read($Heap, n#19, M3.Node.children) } 
            { Seq#Contains(stackNodes#0, $Box(n#19)) } 
            { read($Heap, n#19, M3.Node.marked) } 
            { S#0[$Box(n#19)] } 
          $Is(n#19, Tclass.M3.Node())
            ...
            S#0[$Box(n#19)]
               ...
               && n#19 != t#0
             ==> (forall ch#7: ref :: 
              { read($Heap, ch#7, M3.Node.marked) } 
                { Seq#Contains(read($Heap, n#19, M3.Node.children), $Box(ch#7)) } 
              $Is(ch#7, Tclass.M3.Node?())
                 ==> 
                Seq#Contains(read($Heap, n#19, M3.Node.children), $Box(ch#7)) && ch#7 != null
                 ==> read($Heap, ch#7, M3.Node.marked)));
      free invariant $w$loop#0 ==> true;
      free invariant $w$loop#0
         ==> (forall n#21: ref :: 
          { read($Heap, n#21, M3.Node.marked) } 
            { Seq#Contains(stackNodes#0, $Box(n#21)) } 
          $Is(n#21, Tclass.M3.Node())
             ==> 
            Seq#Contains(stackNodes#0, $Box(n#21)) || n#21 == t#0
             ==> read($Heap, n#21, M3.Node.marked));
      free invariant $w$loop#0
         ==> (forall n#21: ref :: 
          { read($Heap, n#21, M3.Node.children) } 
            { read($Heap, n#21, M3.Node.childrenVisited) } 
            { Seq#Contains(stackNodes#0, $Box(n#21)) } 
          $Is(n#21, Tclass.M3.Node())
             ==> 
            Seq#Contains(stackNodes#0, $Box(n#21)) || n#21 == t#0
             ==> (forall j#3: int :: 
              { $Unbox(Seq#Index(read($Heap, n#21, M3.Node.children), j#3)): ref } 
              true
                 ==> 
                LitInt(0) <= j#3 && j#3 < read($Heap, n#21, M3.Node.childrenVisited)
                 ==> $Unbox(Seq#Index(read($Heap, n#21, M3.Node.children), j#3)): ref == null
                   || read($Heap, 
                    $Unbox(Seq#Index(read($Heap, n#21, M3.Node.children), j#3)): ref, 
                    M3.Node.marked)));
      free invariant $w$loop#0
         ==> 
        $IsAlloc(path#0, Tclass.M3.Path(), old($Heap))
         ==> M3.__default.ReachableVia#canCall(old($Heap), root#0, path#0, t#0, S#0);
      free invariant $w$loop#0 ==> $IsAlloc(path#0, Tclass.M3.Path(), old($Heap));
      free invariant $w$loop#0
         ==> M3.__default.ReachableVia($LS($LS($LZ)), old($Heap), root#0, path#0, t#0, S#0);
      free invariant $w$loop#0
         ==> (forall n#23: ref :: 
          { M3.__default.ReachableVia($LS($LZ), 
              old($Heap), 
              root#0, 
              read(old($Heap), n#23, M3.Node.pathFromRoot), 
              n#23, 
              S#0) } 
            { read($Heap, n#23, M3.Node.pathFromRoot) } 
            { read($Heap, n#23, M3.Node.marked) } 
            { S#0[$Box(n#23)] } 
          $Is(n#23, Tclass.M3.Node())
             ==> 
            S#0[$Box(n#23)]
             ==> 
            read($Heap, n#23, M3.Node.marked)
             ==> (var pth#0 := read($Heap, n#23, M3.Node.pathFromRoot); 
              $IsAlloc(pth#0, Tclass.M3.Path(), old($Heap))
                 ==> M3.__default.ReachableVia#canCall(old($Heap), root#0, pth#0, n#23, S#0)));
      free invariant $w$loop#0
         ==> (forall n#23: ref :: 
          { M3.__default.ReachableVia($LS($LZ), 
              old($Heap), 
          ...
          $Is(n#23, Tclass.M3.Node())
             ==> 
            S#0[$Box(n#23)] && read($Heap, n#23, M3.Node.marked)
             ==> (var pth#0 := read($Heap, n#23, M3.Node.pathFromRoot); 
              $IsAlloc(pth#0, Tclass.M3.Path(), old($Heap))
                 && M3.__default.ReachableVia($LS($LZ), old($Heap), root#0, pth#0, n#23, S#0)));
      free invariant $w$loop#0
         ==> (forall n#25: ref :: 
          { M3.__default.Reachable(old($Heap), root#0, n#25, S#0) } 
            { read($Heap, n#25, M3.Node.marked) } 
            { S#0[$Box(n#25)] } 
          $Is(n#25, Tclass.M3.Node())
            ...
            S#0[$Box(n#25)]
             ==> 
            read($Heap, n#25, M3.Node.marked)
             ==> M3.__default.Reachable#canCall(old($Heap), root#0, n#25, S#0));
      free invariant $w$loop#0
         ==> (forall n#25: ref :: 
          { M3.__default.Reachable(old($Heap), root#0, n#25, S#0) } 
            { read($Heap, n#25, M3.Node.marked) } 
          ...
          $Is(n#25, Tclass.M3.Node())
             ==> 
            S#0[$Box(n#25)] && read($Heap, n#25, M3.Node.marked)
             ==> M3.__default.Reachable(old($Heap), root#0, n#25, S#0));
      free invariant $w$loop#0 ==> true;
      invariant $w$loop#0
         ==> (forall n#27: ref :: 
          { unmarkedNodes#0[$Box(n#27)] } 
            { read($Heap, n#27, M3.Node.marked) } 
            { S#0[$Box(n#27)] } 
          $Is(n#27, Tclass.M3.Node())
             ==> 
            S#0[$Box(n#27)] && !read($Heap, n#27, M3.Node.marked)
             ==> unmarkedNodes#0[$Box(n#27)]);
             ...
             && (Seq#Rank(stackNodes#0) == Seq#Rank($decr_init$loop#01)
               ==> Seq#Length(read($Heap, t#0, M3.Node.children))
                 ...
                 && (Seq#Length(read($Heap, t#0, M3.Node.children))
                   ...
                   ==> true)));
    {
        assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy(257,4): after some loop iterations"} true;
        if (!$w$loop#0)
        {
            ...
            assume {:subsumption 0} t#0 != null;
            assume true;
            assume read($Heap, t#0, M3.Node.childrenVisited)
               <= Seq#Length(read($Heap, t#0, M3.Node.children));
            // Begin Comprehension WF check
            havoc n#12;
            if ($Is(n#12, Tclass.M3.Node()))
            {
                if (S#0[$Box(n#12)])
                {
                }
            ...
            // Begin Comprehension WF check
            havoc n#14;
            if ($Is(n#14, Tclass.M3.Node()))
            {
                if (Seq#Contains(stackNodes#0, $Box(n#14)))
                {
                    assume {:subsumption 0} n#14 != null;
                    assume {:subsumption 0} n#14 != null;
                    assume $IsAlloc(n#14, Tclass.M3.Node(), old($Heap));
                    if (Seq#Length(read($Heap, n#14, M3.Node.children))
                       == Seq#Length(read(old($Heap), n#14, M3.Node.children)))
                    {
                        // Begin Comprehension WF check
                        havoc j#0;
                        if (true)
                        {
                            ...
                            if (LitInt(0) <= j#0 && j#0 < Seq#Length(read($Heap, n#14, M3.Node.children)))
                            {
                                assume {:subsumption 0} n#14 != null;
                                if (j#0 != read($Heap, n#14, M3.Node.childrenVisited))
                                {
                                    assume {:subsumption 0} n#14 != null;
                                    assume {:subsumption 0} 0 <= j#0 && j#0 < Seq#Length(read($Heap, n#14, M3.Node.children));
                                    assume {:subsumption 0} n#14 != null;
                                    assume $IsAlloc(n#14, Tclass.M3.Node(), old($Heap));
                                    assume {:subsumption 0} 0 <= j#0 && j#0 < Seq#Length(read(old($Heap), n#14, M3.Node.children));
                                }
                            }
                        }
            ...
            if ($Is(n#16, Tclass.M3.Node()))
            {
                ...
                if (S#0[$Box(n#16)] && !Seq#Contains(stackNodes#0, $Box(n#16)))
                {
                    ...
                    assume {:subsumption 0} n#16 != null;
                    assume $IsAlloc(n#16, Tclass.M3.Node(), old($Heap));
                }
            }
            ...
            if (0 < Seq#Length(stackNodes#0))
            {
                ...
                assume {:subsumption 0} 0
                     <= read($Heap, $Unbox(Seq#Index(stackNodes#0, LitInt(0))): ref, M3.Node.childrenVisited)
                   && read($Heap, $Unbox(Seq#Index(stackNodes#0, LitInt(0))): ref, M3.Node.childrenVisited)
                     < Seq#Length(read($Heap, $Unbox(Seq#Index(stackNodes#0, LitInt(0))): ref, M3.Node.children));
            }
            ...
            if (true)
            {
                ...
                if (0 < k#0 && k#0 < Seq#Length(stackNodes#0))
                {
                    ...
                    assume {:subsumption 0} $Unbox(Seq#Index(stackNodes#0, k#0)): ref != null;
                    assume {:subsumption 0} 0
                         <= read($Heap, $Unbox(Seq#Index(stackNodes#0, k#0)): ref, M3.Node.childrenVisited)
                       && read($Heap, $Unbox(Seq#Index(stackNodes#0, k#0)): ref, M3.Node.childrenVisited)
                         < Seq#Length(read($Heap, $Unbox(Seq#Index(stackNodes#0, k#0)): ref, M3.Node.children));
            ...
            if (true)
            {
                ...
                if (LitInt(0) <= k#2 && k#2 < Seq#Length(stackNodes#0) - 1)
                {
                    ...
                    assume $IsAlloc($Unbox(Seq#Index(stackNodes#0, k#2)): ref, Tclass.M3.Node(), old($Heap));
                    assume {:subsumption 0} 0 <= k#2 && k#2 < Seq#Length(stackNodes#0);
                    assume {:subsumption 0} $Unbox(Seq#Index(stackNodes#0, k#2)): ref != null;
                    assume {:subsumption 0} 0
                         <= read($Heap, $Unbox(Seq#Index(stackNodes#0, k#2)): ref, M3.Node.childrenVisited)
                       && read($Heap, $Unbox(Seq#Index(stackNodes#0, k#2)): ref, M3.Node.childrenVisited)
                         < Seq#Length(read(old($Heap), $Unbox(Seq#Index(stackNodes#0, k#2)): ref, M3.Node.children));
            ...
            assume true;
            assume read($Heap, root#0, M3.Node.marked);
            // Begin Comprehension WF check
            havoc n#18;
            if ($Is(n#18, Tclass.M3.Node()))
            {
                if (S#0[$Box(n#18)])
                {
                ...
                if (S#0[$Box(n#18)] && read($Heap, n#18, M3.Node.marked))
                {
                }

                if (S#0[$Box(n#18)]
                   && read($Heap, n#18, M3.Node.marked)
                   ...
                   && n#18 != t#0)
                {
                    // Begin Comprehension WF check
                    havoc ch#6;
                    if ($Is(ch#6, Tclass.M3.Node?()))
                    {
                        assume {:subsumption 0} n#18 != null;
                        if (Seq#Contains(read($Heap, n#18, M3.Node.children), $Box(ch#6)))
                        {
                        }

                        if (Seq#Contains(read($Heap, n#18, M3.Node.children), $Box(ch#6)) && ch#6 != null)
                        {
            ...
            if ($Is(n#20, Tclass.M3.Node()))
            {
                ...
                if (Seq#Contains(stackNodes#0, $Box(n#20)) || n#20 == t#0)
                {
                    ...
                    if (read($Heap, n#20, M3.Node.marked))
                    {
                        // Begin Comprehension WF check
                        havoc j#2;
                        if (true)
                        {
                            ...
                            if (LitInt(0) <= j#2 && j#2 < read($Heap, n#20, M3.Node.childrenVisited))
                            {
                                assume {:subsumption 0} n#20 != null;
                                assume {:subsumption 0} 0 <= j#2 && j#2 < Seq#Length(read($Heap, n#20, M3.Node.children));
                                if ($Unbox(Seq#Index(read($Heap, n#20, M3.Node.children), j#2)): ref != null)
                                {
                                    assume {:subsumption 0} n#20 != null;
                                    assume {:subsumption 0} 0 <= j#2 && j#2 < Seq#Length(read($Heap, n#20, M3.Node.children));
                                    assume {:subsumption 0} $Unbox(Seq#Index(read($Heap, n#20, M3.Node.children), j#2)): ref != null;
                                }
                            }
                        }
                      ...
                      LitInt(0) <= j#3 && j#3 < read($Heap, n#21, M3.Node.childrenVisited)
                       ==> $Unbox(Seq#Index(read($Heap, n#21, M3.Node.children), j#3)): ref == null
                         || read($Heap, 
                          $Unbox(Seq#Index(read($Heap, n#21, M3.Node.children), j#3)): ref, 
                          M3.Node.marked)));
            if ($IsAlloc(path#0, Tclass.M3.Path(), old($Heap)))
            {
                ##source#1 := root#0;
                ##p#0 := path#0;
                ##sink#1 := t#0;
                ##S#1 := S#0;
                assume $IsAlloc(root#0, Tclass.M3.Node(), old($Heap));
                assume $IsAlloc(path#0, Tclass.M3.Path(), old($Heap));
                assume $IsAlloc(t#0, Tclass.M3.Node(), old($Heap));
                assume $IsAlloc(S#0, TSet(Tclass.M3.Node()), old($Heap));
            ...
            if ($Is(n#22, Tclass.M3.Node()))
            {
                ...
                if (S#0[$Box(n#22)] && read($Heap, n#22, M3.Node.marked))
                {
                    havoc pth#Z#0;
                    assume $Is(pth#Z#0, Tclass.M3.Path());
                    assume {:subsumption 0} n#22 != null;
                    assume let#0#0#0 == read($Heap, n#22, M3.Node.pathFromRoot);
                    assume true;
                    // CheckWellformedWithResult: any expression
                    assume $Is(let#0#0#0, Tclass.M3.Path());
                    assume pth#Z#0 == let#0#0#0;
                    if ($IsAlloc(pth#Z#0, Tclass.M3.Path(), old($Heap)))
                    {
                        ##source#2 := root#0;
                        ##p#1 := pth#Z#0;
                        ##sink#2 := n#22;
                        ##S#2 := S#0;
                        assume $IsAlloc(root#0, Tclass.M3.Node(), old($Heap));
                        assume $IsAlloc(pth#Z#0, Tclass.M3.Path(), old($Heap));
                        assume $IsAlloc(n#22, Tclass.M3.Node(), old($Heap));
                        assume $IsAlloc(S#0, TSet(Tclass.M3.Node()), old($Heap));
                        assume M3.__default.ReachableVia#canCall(old($Heap), root#0, pth#Z#0, n#22, S#0);
                    }
                }
            }
            ...
            if ($Is(n#24, Tclass.M3.Node()))
            {
                ...
                if (S#0[$Box(n#24)] && read($Heap, n#24, M3.Node.marked))
                {
                    ##source#3 := root#0;
                    ##sink#3 := n#24;
                    ##S#3 := S#0;
                    assume $IsAlloc(root#0, Tclass.M3.Node(), old($Heap));
                    assume $IsAlloc(n#24, Tclass.M3.Node(), old($Heap));
                    assume $IsAlloc(S#0, TSet(Tclass.M3.Node()), old($Heap));
                    assume M3.__default.Reachable#canCall(old($Heap), root#0, n#24, S#0);
                }
            }
            ...
            if ($Is(n#26, Tclass.M3.Node()))
            {
                ...
                if (S#0[$Box(n#26)] && !read($Heap, n#26, M3.Node.marked))
                {
                }
            }
        ...
        $decr$loop#02 := Seq#Length(read($Heap, t#0, M3.Node.children))
           - read($Heap, t#0, M3.Node.childrenVisited);
        // ----- if statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy(261,7)
        assume t#0 != null;
        assume t#0 != null;
        assume true;
        if (read($Heap, t#0, M3.Node.childrenVisited)
           == Seq#Length(read($Heap, t#0, M3.Node.children)))
        {
            // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](111,27)
            assume t#0 != null;
            assume true;
            assume $_Frame[t#0, M3.Node.childrenVisited];
            assume true;
            assume $Is(LitInt(0), Tclass._System.nat());
            $rhs#0_0_0 := LitInt(0);
            $Heap := update($Heap, t#0, M3.Node.childrenVisited, $rhs#0_0_0);
            assume $IsGoodHeap($Heap);
            assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](111,30)"} true;
            // ----- if statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](112,9)
            assume true;
            if (p#0 == null)
            {
                // ----- break statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](112,24)
                goto after_0;
            }
            else
            {
            ...
            // ----- update statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](114,26)
            assume true;
            assume true;
            assume p#0 != null;
            assume true;
            $obj2 := p#0;
            assume $_Frame[$obj2, M3.Node.children];
            assume true;
            assume $Is(p#0, Tclass.M3.Node());
            $rhs#0_0_1 := p#0;
            assume p#0 != null;
            assume p#0 != null;
            assume 0 <= read($Heap, p#0, M3.Node.childrenVisited)
               && read($Heap, p#0, M3.Node.childrenVisited)
                 < Seq#Length(read($Heap, p#0, M3.Node.children));
            assume true;
            $rhs#0_0_2 := $Unbox(Seq#Index(read($Heap, p#0, M3.Node.children), read($Heap, p#0, M3.Node.childrenVisited))): ref;
            assume p#0 != null;
            assume p#0 != null;
            assume 0 <= read($Heap, p#0, M3.Node.childrenVisited)
               && read($Heap, p#0, M3.Node.childrenVisited)
                 < Seq#Length(read($Heap, p#0, M3.Node.children));
            assume true;
            $rhs#0_0_3 := Seq#Update(read($Heap, p#0, M3.Node.children), 
              read($Heap, p#0, M3.Node.childrenVisited), 
              $Box(t#0));
            t#0 := $rhs#0_0_1;
            p#0 := $rhs#0_0_2;
            $Heap := update($Heap, $obj2, M3.Node.children, $rhs#0_0_3);
            assume $IsGoodHeap($Heap);
            assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](114,96)"} true;
            // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](115,20)
            assume true;
            assume 0 <= Seq#Length(stackNodes#0) - 1
               && Seq#Length(stackNodes#0) - 1 <= Seq#Length(stackNodes#0);
            assume true;
            stackNodes#0 := Seq#Take(stackNodes#0, Seq#Length(stackNodes#0) - 1);
            assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](115,52)"} true;
            // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](116,27)
            assume t#0 != null;
            assume true;
            assume $_Frame[t#0, M3.Node.childrenVisited];
            assume t#0 != null;
            assume true;
            assume $Is(read($Heap, t#0, M3.Node.childrenVisited) + 1, Tclass._System.nat());
            $rhs#0_0_4 := read($Heap, t#0, M3.Node.childrenVisited) + 1;
            $Heap := update($Heap, t#0, M3.Node.childrenVisited, $rhs#0_0_4);
            assume $IsGoodHeap($Heap);
            assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](116,50)"} true;
            // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M3](225,14)
            assume true;
            assume t#0 != null;
            assume true;
            path#0 := read($Heap, t#0, M3.Node.pathFromRoot);
            assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M3](225,30)"} true;
        }
        else
        {
            // ----- if statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy(262,14)
            assume t#0 != null;
            assume t#0 != null;
            assume 0 <= read($Heap, t#0, M3.Node.childrenVisited)
               && read($Heap, t#0, M3.Node.childrenVisited)
                 < Seq#Length(read($Heap, t#0, M3.Node.children));
            if ($Unbox(Seq#Index(read($Heap, t#0, M3.Node.children), read($Heap, t#0, M3.Node.childrenVisited))): ref
               != null)
            {
                assume t#0 != null;
                assume t#0 != null;
                assume 0 <= read($Heap, t#0, M3.Node.childrenVisited)
                   && read($Heap, t#0, M3.Node.childrenVisited)
                     < Seq#Length(read($Heap, t#0, M3.Node.children));
                assume $Unbox(Seq#Index(read($Heap, t#0, M3.Node.children), read($Heap, t#0, M3.Node.childrenVisited))): ref
                   != null;
            }
            ...
            if ($Unbox(Seq#Index(read($Heap, t#0, M3.Node.children), read($Heap, t#0, M3.Node.childrenVisited))): ref
                 == null
               || read($Heap, 
                $Unbox(Seq#Index(read($Heap, t#0, M3.Node.children), read($Heap, t#0, M3.Node.childrenVisited))): ref, 
                M3.Node.marked))
            {
                // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](120,27)
                assume t#0 != null;
                assume true;
                assume $_Frame[t#0, M3.Node.childrenVisited];
                assume t#0 != null;
                assume true;
                assume $Is(read($Heap, t#0, M3.Node.childrenVisited) + 1, Tclass._System.nat());
                $rhs#0_1_0 := read($Heap, t#0, M3.Node.childrenVisited) + 1;
                $Heap := update($Heap, t#0, M3.Node.childrenVisited, $rhs#0_1_0);
                assume $IsGoodHeap($Heap);
                assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](120,50)"} true;
            }
            else
            {
                // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M3](230,14)
                assume true;
                assume true;
                path#0 := #M3.Path.Extend(path#0, t#0);
                assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M3](230,36)"} true;
                // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](124,20)
                assume true;
                assume true;
                stackNodes#0 := Seq#Append(stackNodes#0, Seq#Build(Seq#Empty(): Seq Box, $Box(t#0)));
                assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](124,38)"} true;
                // ----- update statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](125,26)
                assume true;
                assume true;
                assume t#0 != null;
                assume true;
                ...
                assume 0 <= read($Heap, t#0, M3.Node.childrenVisited)
                   && read($Heap, t#0, M3.Node.childrenVisited)
                     < Seq#Length(read($Heap, t#0, M3.Node.children));
                assume true;
                assume $Is($Unbox(Seq#Index(read($Heap, t#0, M3.Node.children), read($Heap, t#0, M3.Node.childrenVisited))): ref, 
                  Tclass.M3.Node());
                $rhs#0_0 := $Unbox(Seq#Index(read($Heap, t#0, M3.Node.children), read($Heap, t#0, M3.Node.childrenVisited))): ref;
                assume true;
                $rhs#0_1 := t#0;
                assume t#0 != null;
                assume t#0 != null;
                assume 0 <= read($Heap, t#0, M3.Node.childrenVisited)
                   && read($Heap, t#0, M3.Node.childrenVisited)
                     < Seq#Length(read($Heap, t#0, M3.Node.children));
                assume true;
                $rhs#0_2 := Seq#Update(read($Heap, t#0, M3.Node.children), 
                  read($Heap, t#0, M3.Node.childrenVisited), 
                  $Box(p#0));
                t#0 := $rhs#0_0;
                p#0 := $rhs#0_1;
                $Heap := update($Heap, $obj2, M3.Node.children, $rhs#0_2);
                assume $IsGoodHeap($Heap);
                assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](125,96)"} true;
                // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](126,18)
                assume t#0 != null;
                assume true;
                assume $_Frame[t#0, M3.Node.marked];
                assume true;
                $rhs#0_3 := Lit(true);
                $Heap := update($Heap, t#0, M3.Node.marked, $rhs#0_3);
                assume $IsGoodHeap($Heap);
                assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M1][M2][M3](126,24)"} true;
                // ----- assert statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M2][M3](169,9)
                assume true;
                assume !Seq#Contains(stackNodes#0, $Box(t#0));
                // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M3](232,24)
                assume t#0 != null;
                assume true;
                assume $_Frame[t#0, M3.Node.pathFromRoot];
                assume true;
                $rhs#0_4 := path#0;
                $Heap := update($Heap, t#0, M3.Node.pathFromRoot, $rhs#0_4);
                assume $IsGoodHeap($Heap);
                assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M3](232,30)"} true;
                // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy(265,23)
                assume true;
                assume true;
                unmarkedNodes#0 := Set#Difference(unmarkedNodes#0, Set#UnionOne(Set#Empty(): Set Box, $Box(t#0)));
                assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy(265,44)"} true;
            }
        }

        // ----- loop termination check ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy(257,5)
  ...
  after_0:
    // ----- assert statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M3](237,5)
    assume {:subsumption 0} root#0 != null;
    if (read($Heap, root#0, M3.Node.marked))
    {
        // Begin Comprehension WF check
        havoc n#28;
        if ($Is(n#28, Tclass.M3.Node()))
        {
            if (S#0[$Box(n#28)])
            {
            ...
            if (S#0[$Box(n#28)] && read($Heap, n#28, M3.Node.marked))
            {
                // Begin Comprehension WF check
                havoc ch#8;
                if ($Is(ch#8, Tclass.M3.Node?()))
                {
                    assume {:subsumption 0} n#28 != null;
                    if (Seq#Contains(read($Heap, n#28, M3.Node.children), $Box(ch#8)))
                    {
                    }

                    if (Seq#Contains(read($Heap, n#28, M3.Node.children), $Box(ch#8)) && ch#8 != null)
                    {
    ...
    assume read($Heap, root#0, M3.Node.marked)
       && (forall n#29: ref :: 
        { read($Heap, n#29, M3.Node.children) } 
          { read($Heap, n#29, M3.Node.marked) } 
          { S#0[$Box(n#29)] } 
        $Is(n#29, Tclass.M3.Node())
           ==> 
          S#0[$Box(n#29)] && read($Heap, n#29, M3.Node.marked)
           ==> (forall ch#9: ref :: 
            { read($Heap, ch#9, M3.Node.marked) } 
              { Seq#Contains(read($Heap, n#29, M3.Node.children), $Box(ch#9)) } 
            $Is(ch#9, Tclass.M3.Node?())
               ==> 
              Seq#Contains(read($Heap, n#29, M3.Node.children), $Box(ch#9)) && ch#9 != null
               ==> read($Heap, ch#9, M3.Node.marked)));
    // ----- assert statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny1/SchorrWaite-stages.dfy[M3](240,5)
    // Begin Comprehension WF check
    havoc n#30;
    if ($Is(n#30, Tclass.M3.Node()))
    {
        if (S#0[$Box(n#30)])
        {
            assume {:subsumption 0} n#30 != null;
        }

        if (S#0[$Box(n#30)] && read($Heap, n#30, M3.Node.marked))
        {
            ##source#4 := root#0;
            ##sink#4 := n#30;
            ##S#4 := S#0;
            assume $IsAlloc(root#0, Tclass.M3.Node(), old($Heap));
            assume $IsAlloc(n#30, Tclass.M3.Node(), old($Heap));
            assume $IsAlloc(S#0, TSet(Tclass.M3.Node()), old($Heap));
            assume M3.__default.Reachable#canCall(old($Heap), root#0, n#30, S#0);
        }
    }

    // End Comprehension WF check
    assume (forall n#31: ref :: 
      { M3.__default.Reachable(old($Heap), root#0, n#31, S#0) } 
        { read($Heap, n#31, M3.Node.marked) } 
        { S#0[$Box(n#31)] } 
      $Is(n#31, Tclass.M3.Node())
        ...
        S#0[$Box(n#31)]
         ==> 
        read($Heap, n#31, M3.Node.marked)
         ==> M3.__default.Reachable#canCall(old($Heap), root#0, n#31, S#0));
    assume (forall n#31: ref :: 
      { M3.__default.Reachable(old($Heap), root#0, n#31, S#0) } 
        { read($Heap, n#31, M3.Node.marked) } 
        { S#0[$Box(n#31)] } 
      $Is(n#31, Tclass.M3.Node())
         ==> 
        S#0[$Box(n#31)] && read($Heap, n#31, M3.Node.marked)
         ==> M3.__default.Reachable(old($Heap), root#0, n#31, S#0));
}
...
function M3.__default.Reachable($heap: Heap, source#0: ref, sink#0: ref, S#0: Set Box) : bool;

function M3.__default.Reachable#canCall($heap: Heap, source#0: ref, sink#0: ref, S#0: Set Box) : bool;
...
// consequence axiom for M3.__default.Reachable
...
function M3.__default.Reachable#requires(Heap, ref, ref, Set Box) : bool;
...
procedure CheckWellformed$$M3.__default.Reachable(source#0: ref where $Is(source#0, Tclass.M3.Node()), 
    sink#0: ref where $Is(sink#0, Tclass.M3.Node()), 
    S#0: Set Box where $Is(S#0, TSet(Tclass.M3.Node())));
...
function M3.__default.ReachableVia#canCall($heap: Heap, source#0: ref, p#0: DatatypeType, sink#0: ref, S#0: Set Box) : bool;
...
function M3.__default.ReachableVia#requires(LayerType, Heap, ref, DatatypeType, ref, Set Box) : bool;
       ...
       ==> (!M3.Path.Empty_q(p#0)
           ==> (var n#1 := M3.Path._h1(p#0); 
            (var prefix#1 := M3.Path._h0(p#0); 
              S#0[$Box(n#1)]
                 ==> 
                Seq#Contains(read($Heap, n#1, M3.Node.children), $Box(sink#0))
           ...
           == (if M3.Path.Empty_q(p#0)
             then source#0 == sink#0
              ...
              (var prefix#0 := M3.Path._h0(p#0); 
                S#0[$Box(n#0)]
                   && Seq#Contains(read($Heap, n#0, M3.Node.children), $Box(sink#0))
                   && M3.__default.ReachableVia($ly, $Heap, source#0, prefix#0, n#0, S#0)))));

procedure CheckWellformed$$M3.__default.ReachableVia(source#0: ref where $Is(source#0, Tclass.M3.Node()), 
...
axiom (forall $o: ref, $h: Heap :: 
  ...
  $IsAlloc($o, Tclass.M3.Node?(), $h) <==> $o == null || read($h, $o, alloc));

const M3.Node.pathFromRoot: Field DatatypeType;

// Node.pathFromRoot: Type axiom
axiom (forall $h: Heap, $o: ref :: 
  { read($h, $o, M3.Node.pathFromRoot) } 
  $IsGoodHeap($h) && $o != null && dtype($o) == Tclass.M3.Node?()
     ==> $Is(read($h, $o, M3.Node.pathFromRoot), Tclass.M3.Path()));
...
axiom (forall $h: Heap, $o: ref :: 
  ...
  $IsGoodHeap($h)
      ...
      $o != null
       ...
       && read($h, $o, alloc)
     ==> $IsAlloc(read($h, $o, M3.Node.pathFromRoot), Tclass.M3.Path(), $h));

const M3.Node.children: Field (Seq Box);

// Node.children: Type axiom
axiom (forall $h: Heap, $o: ref :: 
  { read($h, $o, M3.Node.children) } 
  $IsGoodHeap($h) && $o != null && dtype($o) == Tclass.M3.Node?()
     ==> $Is(read($h, $o, M3.Node.children), TSeq(Tclass.M3.Node?())));
...
axiom (forall $h: Heap, $o: ref :: 
  ...
  $IsGoodHeap($h)
      ...
      $o != null
       && dtype($o) == Tclass.M3.Node?()
       && read($h, $o, alloc)
     ==> $IsAlloc(read($h, $o, M3.Node.children), TSeq(Tclass.M3.Node?()), $h));

const M3.Node.marked: Field bool;

// Node.marked: Type axiom
axiom (forall $h: Heap, $o: ref :: 
  { read($h, $o, M3.Node.marked) } 
  $IsGoodHeap($h) && $o != null && dtype($o) == Tclass.M3.Node?()
     ==> $Is(read($h, $o, M3.Node.marked), TBool));
...
axiom (forall $h: Heap, $o: ref :: 
  ...
  $IsGoodHeap($h)
      ...
      $o != null
       && dtype($o) == Tclass.M3.Node?()
       && read($h, $o, alloc)
     ==> $IsAlloc(read($h, $o, M3.Node.marked), TBool, $h));

const M3.Node.childrenVisited: Field int;

// Node.childrenVisited: Type axiom
axiom (forall $h: Heap, $o: ref :: 
  { read($h, $o, M3.Node.childrenVisited) } 
  $IsGoodHeap($h) && $o != null && dtype($o) == Tclass.M3.Node?()
     ==> $Is(read($h, $o, M3.Node.childrenVisited), Tclass._System.nat()));
...
axiom (forall $h: Heap, $o: ref :: 
  ...
  $IsGoodHeap($h)
      ...
      $o != null
       && dtype($o) == Tclass.M3.Node?()
       && read($h, $o, alloc)
     ==> $IsAlloc(read($h, $o, M3.Node.childrenVisited), Tclass._System.nat(), $h));

// M3.Node: subset type $Is
axiom (forall c#0: ref :: 
  { $Is(c#0, Tclass.M3.Node()) } 
  $Is(c#0, Tclass.M3.Node()) <==> $Is(c#0, Tclass.M3.Node?()) && c#0 != null);

// M3.Node: subset type $IsAlloc
axiom (forall c#0: ref, $h: Heap :: 
...
axiom (forall a#5#0#0: DatatypeType, a#5#1#0: ref :: 
  { #M3.Path.Extend(a#5#0#0, a#5#1#0) } 
  DatatypeCtorId(#M3.Path.Extend(a#5#0#0, a#5#1#0)) == ##M3.Path.Extend);
...
axiom (forall d: DatatypeType :: 
  ...
  M3.Path.Extend_q(d)
     ==> (exists a#6#0#0: DatatypeType, a#6#1#0: ref :: 
      d == #M3.Path.Extend(a#6#0#0, a#6#1#0)));

// Constructor $Is
axiom (forall a#7#0#0: DatatypeType, a#7#1#0: ref :: 
  { $Is(#M3.Path.Extend(a#7#0#0, a#7#1#0), Tclass.M3.Path()) } 
  $Is(#M3.Path.Extend(a#7#0#0, a#7#1#0), Tclass.M3.Path())
     <==> $Is(a#7#0#0, Tclass.M3.Path()) && $Is(a#7#1#0, Tclass.M3.Node()));

// Constructor $IsAlloc
axiom (forall a#8#0#0: DatatypeType, a#8#1#0: ref, $h: Heap :: 
  { $IsAlloc(#M3.Path.Extend(a#8#0#0, a#8#1#0), Tclass.M3.Path(), $h) } 
  $IsGoodHeap($h)
     ==> ($IsAlloc(#M3.Path.Extend(a#8#0#0, a#8#1#0), Tclass.M3.Path(), $h)
       <==> $IsAlloc(a#8#0#0, Tclass.M3.Path(), $h)
         && $IsAlloc(a#8#1#0, Tclass.M3.Node(), $h)));
...
axiom (forall a#9#0#0: DatatypeType, a#9#1#0: ref :: 
  { #M3.Path.Extend(Lit(a#9#0#0), Lit(a#9#1#0)) } 
  #M3.Path.Extend(Lit(a#9#0#0), Lit(a#9#1#0))
     == Lit(#M3.Path.Extend(a#9#0#0, a#9#1#0)));
...
axiom (forall a#10#0#0: DatatypeType, a#10#1#0: ref :: 
  ...
  M3.Path._h0(#M3.Path.Extend(a#10#0#0, a#10#1#0)) == a#10#0#0);

// Inductive rank
axiom (forall a#11#0#0: DatatypeType, a#11#1#0: ref :: 
  { #M3.Path.Extend(a#11#0#0, a#11#1#0) } 
  DtRank(a#11#0#0) < DtRank(#M3.Path.Extend(a#11#0#0, a#11#1#0)));

function M3.Path._h1(DatatypeType) : ref;

// Constructor injectivity
axiom (forall a#12#0#0: DatatypeType, a#12#1#0: ref :: 
  { #M3.Path.Extend(a#12#0#0, a#12#1#0) } 
  M3.Path._h1(#M3.Path.Extend(a#12#0#0, a#12#1#0)) == a#12#1#0);
...
axiom (forall a: DatatypeType, b: DatatypeType :: 
  ...
  M3.Path.Empty_q(a) && M3.Path.Empty_q(b) ==> (M3.Path#Equal(a, b) <==> true));
