// Dafny 3.0.0.30204
// Command Line Options: -compile:0 -noVerify /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny3/WideTrees.dfy -print:./WideTrees.bpl
...
// function declaration for _module._default.BigTree
function _module.__default.BigTree($ly: LayerType) : DatatypeType;

function _module.__default.BigTree#canCall() : bool;

// layer synonym axiom
axiom (forall $ly: LayerType :: 
  { _module.__default.BigTree($LS($ly)) } 
  _module.__default.BigTree($LS($ly)) == _module.__default.BigTree($ly));

// fuel synonym axiom
axiom (forall $ly: LayerType :: 
  { _module.__default.BigTree(AsFuelBottom($ly)) } 
  _module.__default.BigTree($ly) == _module.__default.BigTree($LZ));

// consequence axiom for _module.__default.BigTree
axiom 8 <= $FunctionContextHeight
   ==> (forall $ly: LayerType :: 
    { _module.__default.BigTree($ly) } 
    _module.__default.BigTree#canCall() || 8 != $FunctionContextHeight
       ==> $Is(_module.__default.BigTree($ly), Tclass._module.Tree()));

function _module.__default.BigTree#requires(LayerType) : bool;

// #requires axiom for _module.__default.BigTree
axiom (forall $ly: LayerType :: 
  { _module.__default.BigTree#requires($ly) } 
  _module.__default.BigTree#requires($ly) == true);

// definition axiom for _module.__default.BigTree (revealed)
axiom 8 <= $FunctionContextHeight
   ==> (forall $ly: LayerType :: 
    { _module.__default.BigTree($LS($ly)) } 
    _module.__default.BigTree#canCall() || 8 != $FunctionContextHeight
       ==> _module.__default.BigTrees#canCall()
         && _module.__default.BigTree($LS($ly))
           == Lit(#_module.Tree.Node(Lit(_module.__default.BigTrees($ly)))));

procedure CheckWellformed$$_module.__default.BigTree();
  free requires 8 == $FunctionContextHeight;
  modifies $Heap, $Tick;



implementation CheckWellformed$$_module.__default.BigTree()
{
  ...
  var b$reqreads#0: bool;
    ...
    // AddWellformednessCheck for function BigTree
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny3/WideTrees.dfy(8,9): initial state"} true;
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    if (*)
    {
        assume $Is(_module.__default.BigTree($LS($LZ)), Tclass._module.Tree());
        assume false;
    }
    else
    {
        ...
        assume _module.__default.BigTrees#canCall();
        assume _module.__default.BigTree($LS($LZ))
           == Lit(#_module.Tree.Node(Lit(_module.__default.BigTrees($LS($LZ)))));
        assume _module.__default.BigTrees#canCall();
...
// function declaration for _module._default.BigTrees
function _module.__default.BigTrees($ly: LayerType) : DatatypeType;

function _module.__default.BigTrees#canCall() : bool;

// layer synonym axiom
axiom (forall $ly: LayerType :: 
  { _module.__default.BigTrees($LS($ly)) } 
  _module.__default.BigTrees($LS($ly)) == _module.__default.BigTrees($ly));

// fuel synonym axiom
axiom (forall $ly: LayerType :: 
  { _module.__default.BigTrees(AsFuelBottom($ly)) } 
  _module.__default.BigTrees($ly) == _module.__default.BigTrees($LZ));

// consequence axiom for _module.__default.BigTrees
axiom 8 <= $FunctionContextHeight
   ==> (forall $ly: LayerType :: 
    { _module.__default.BigTrees($ly) } 
    _module.__default.BigTrees#canCall() || 8 != $FunctionContextHeight
       ==> $Is(_module.__default.BigTrees($ly), Tclass._module.Stream(Tclass._module.Tree())));

function _module.__default.BigTrees#requires(LayerType) : bool;

// #requires axiom for _module.__default.BigTrees
axiom (forall $ly: LayerType :: 
  { _module.__default.BigTrees#requires($ly) } 
  _module.__default.BigTrees#requires($ly) == true);

// definition axiom for _module.__default.BigTrees (revealed)
axiom 8 <= $FunctionContextHeight
   ==> (forall $ly: LayerType :: 
    { _module.__default.BigTrees($LS($ly)) } 
    _module.__default.BigTrees#canCall() || 8 != $FunctionContextHeight
       ==> _module.__default.BigTree#canCall()
         && _module.__default.BigTrees#canCall()
         && _module.__default.BigTrees($LS($ly))
           == Lit(#_module.Stream.SCons($Box(Lit(_module.__default.BigTree($ly))), Lit(_module.__default.BigTrees($ly)))));

procedure CheckWellformed$$_module.__default.BigTrees();
  free requires 8 == $FunctionContextHeight;
  modifies $Heap, $Tick;



implementation CheckWellformed$$_module.__default.BigTrees()
{
  var $_Frame: <beta>[ref,Field beta]bool;
  var b$reqreads#0: bool;
  var b$reqreads#1: bool;
    ...
    // AddWellformednessCheck for function BigTrees
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny3/WideTrees.dfy(12,9): initial state"} true;
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    if (*)
    {
        assume $Is(_module.__default.BigTrees($LS($LZ)), 
    ...
    else
    {
        $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
          $o != null && read($Heap, $o, alloc) ==> false);
        b$reqreads#0 := (forall<alpha> $o: ref, $f: Field alpha :: false ==> $_Frame[$o, $f]);
        assume _module.__default.BigTree#canCall();
        assume _module.Tree.Node_q(Lit(_module.__default.BigTree($LS($LZ))));
        b$reqreads#1 := (forall<alpha> $o: ref, $f: Field alpha :: false ==> $_Frame[$o, $f]);
        assume _module.__default.BigTrees#canCall();
        assume _module.__default.BigTrees($LS($LZ))
           == Lit(#_module.Stream.SCons($Box(Lit(_module.__default.BigTree($LS($LZ)))), 
              Lit(_module.__default.BigTrees($LS($LZ)))));
        assume _module.__default.BigTree#canCall() && _module.__default.BigTrees#canCall();
        // CheckWellformedWithResult: any expression
        assume $Is(_module.__default.BigTrees($LS($LZ)), 
          Tclass._module.Stream(Tclass._module.Tree()));
        assert b$reqreads#0;
...
axiom 9 <= $FunctionContextHeight
   ==> (forall t#0: DatatypeType :: 
    ...
    _module.__default.HasBoundedHeight#canCall(t#0)
         || (9 != $FunctionContextHeight && $Is(t#0, Tclass._module.Tree()))
       ==> true);
...
implementation CheckWellformed$$_module.__default.HasBoundedHeight(t#0: DatatypeType)
{
  ...
  var b$reqreads#0: bool;
    ...
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny3/WideTrees.dfy(19,10): initial state"} true;
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    if (*)
    {
...
implementation CheckWellformed$$_module.__default.LowerThan(s#0: DatatypeType, n#0: int)
{
  ...
  var b$reqreads#1: bool;
    ...
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny3/WideTrees.dfy(23,19): initial state"} true;
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    if (*)
    {
...
// function declaration for _module._default.SmallTree
function _module.__default.SmallTree($ly: LayerType, n#0: int) : DatatypeType;

function _module.__default.SmallTree#canCall(n#0: int) : bool;

// layer synonym axiom
axiom (forall $ly: LayerType, n#0: int :: 
  { _module.__default.SmallTree($LS($ly), n#0) } 
  _module.__default.SmallTree($LS($ly), n#0)
     == _module.__default.SmallTree($ly, n#0));

// fuel synonym axiom
axiom (forall $ly: LayerType, n#0: int :: 
  { _module.__default.SmallTree(AsFuelBottom($ly), n#0) } 
  _module.__default.SmallTree($ly, n#0) == _module.__default.SmallTree($LZ, n#0));

// consequence axiom for _module.__default.SmallTree
axiom 5 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, n#0: int :: 
    { _module.__default.SmallTree($ly, n#0) } 
    _module.__default.SmallTree#canCall(n#0)
         || (5 != $FunctionContextHeight && LitInt(0) <= n#0)
       ==> $Is(_module.__default.SmallTree($ly, n#0), Tclass._module.Tree()));

function _module.__default.SmallTree#requires(LayerType, int) : bool;

// #requires axiom for _module.__default.SmallTree
axiom (forall $ly: LayerType, n#0: int :: 
  { _module.__default.SmallTree#requires($ly, n#0) } 
  LitInt(0) <= n#0 ==> _module.__default.SmallTree#requires($ly, n#0) == true);

// definition axiom for _module.__default.SmallTree (revealed)
axiom 5 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, n#0: int :: 
    { _module.__default.SmallTree($LS($ly), n#0) } 
    _module.__default.SmallTree#canCall(n#0)
         || (5 != $FunctionContextHeight && LitInt(0) <= n#0)
       ==> _module.__default.SmallTrees#canCall(n#0)
         && _module.__default.SmallTree($LS($ly), n#0)
           == #_module.Tree.Node(_module.__default.SmallTrees($ly, n#0)));

procedure CheckWellformed$$_module.__default.SmallTree(n#0: int where LitInt(0) <= n#0);
  free requires 5 == $FunctionContextHeight;
  modifies $Heap, $Tick;



implementation CheckWellformed$$_module.__default.SmallTree(n#0: int)
{
  var $_Frame: <beta>[ref,Field beta]bool;
  var ##n#0: int;
  var b$reqreads#0: bool;
    ...
    // AddWellformednessCheck for function SmallTree
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny3/WideTrees.dfy(32,9): initial state"} true;
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    if (*)
    {
        assume $Is(_module.__default.SmallTree($LS($LZ), n#0), Tclass._module.Tree());
        assume false;
    }
    else
    {
        ...
        b$reqreads#0 := (forall<alpha> $o: ref, $f: Field alpha :: false ==> $_Frame[$o, $f]);
        assert 0 <= LitInt(1) || LitInt(0) == LitInt(1);
        assert 0 <= n#0 || LitInt(0) < LitInt(1) || LitInt(-1) == n#0;
        assert LitInt(0) < LitInt(1) || (LitInt(0) == LitInt(1) && LitInt(-1) < n#0);
        assume _module.__default.SmallTrees#canCall(n#0);
        assume _module.__default.SmallTree($LS($LZ), n#0)
           == #_module.Tree.Node(_module.__default.SmallTrees($LS($LZ), n#0));
        assume _module.__default.SmallTrees#canCall(n#0);
        // CheckWellformedWithResult: any expression
        assume $Is(_module.__default.SmallTree($LS($LZ), n#0), Tclass._module.Tree());
        assert b$reqreads#0;
...
// function declaration for _module._default.SmallTrees
function _module.__default.SmallTrees($ly: LayerType, n#0: int) : DatatypeType;

function _module.__default.SmallTrees#canCall(n#0: int) : bool;

// layer synonym axiom
axiom (forall $ly: LayerType, n#0: int :: 
  { _module.__default.SmallTrees($LS($ly), n#0) } 
  _module.__default.SmallTrees($LS($ly), n#0)
     == _module.__default.SmallTrees($ly, n#0));

// fuel synonym axiom
axiom (forall $ly: LayerType, n#0: int :: 
  { _module.__default.SmallTrees(AsFuelBottom($ly), n#0) } 
  _module.__default.SmallTrees($ly, n#0) == _module.__default.SmallTrees($LZ, n#0));

// consequence axiom for _module.__default.SmallTrees
axiom 5 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, n#0: int :: 
    { _module.__default.SmallTrees($ly, n#0) } 
    _module.__default.SmallTrees#canCall(n#0)
         || (5 != $FunctionContextHeight && LitInt(0) <= n#0)
       ==> $Is(_module.__default.SmallTrees($ly, n#0), 
        Tclass._module.Stream(Tclass._module.Tree())));

function _module.__default.SmallTrees#requires(LayerType, int) : bool;

// #requires axiom for _module.__default.SmallTrees
axiom (forall $ly: LayerType, n#0: int :: 
  { _module.__default.SmallTrees#requires($ly, n#0) } 
  LitInt(0) <= n#0 ==> _module.__default.SmallTrees#requires($ly, n#0) == true);

// definition axiom for _module.__default.SmallTrees (revealed)
axiom 5 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, n#0: int :: 
    { _module.__default.SmallTrees($LS($ly), n#0) } 
    _module.__default.SmallTrees#canCall(n#0)
         || (5 != $FunctionContextHeight && LitInt(0) <= n#0)
       ==> (n#0 != LitInt(0)
           ==> _module.__default.SmallTree#canCall(n#0 - 1)
             && _module.__default.SmallTrees#canCall(n#0))
         && _module.__default.SmallTrees($LS($ly), n#0)
           == (if n#0 == LitInt(0)
             then #_module.Stream.SNil()
             else #_module.Stream.SCons($Box(_module.__default.SmallTree($ly, n#0 - 1)), 
              _module.__default.SmallTrees($ly, n#0))));

procedure CheckWellformed$$_module.__default.SmallTrees(n#0: int where LitInt(0) <= n#0);
  free requires 5 == $FunctionContextHeight;
  modifies $Heap, $Tick;



implementation CheckWellformed$$_module.__default.SmallTrees(n#0: int)
{
  var $_Frame: <beta>[ref,Field beta]bool;
  var ##n#0: int;
  var ##n#1: int;
  ...
  var b$reqreads#1: bool;
    ...
    // AddWellformednessCheck for function SmallTrees
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny3/WideTrees.dfy(36,9): initial state"} true;
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    if (*)
    {
        assume $Is(_module.__default.SmallTrees($LS($LZ), n#0), 
          Tclass._module.Stream(Tclass._module.Tree()));
        assume false;
    }
    else
    {
        ...
        if (n#0 == LitInt(0))
        {
            assume _module.__default.SmallTrees($LS($LZ), n#0) == Lit(#_module.Stream.SNil());
            assume true;
            // CheckWellformedWithResult: any expression
            assume $Is(_module.__default.SmallTrees($LS($LZ), n#0), 
              Tclass._module.Stream(Tclass._module.Tree()));
        ...
        else
        {
            ...
            assume $IsAlloc(##n#0, Tclass._System.nat(), $Heap);
            b$reqreads#0 := (forall<alpha> $o: ref, $f: Field alpha :: false ==> $_Frame[$o, $f]);
            assume _module.__default.SmallTree#canCall(n#0 - 1);
            assume _module.Tree.Node_q(_module.__default.SmallTree($LS($LZ), n#0 - 1));
            ##n#1 := n#0;
            // assume allocatedness for argument to function
            assume $IsAlloc(##n#1, Tclass._System.nat(), $Heap);
            b$reqreads#1 := (forall<alpha> $o: ref, $f: Field alpha :: false ==> $_Frame[$o, $f]);
            assume _module.__default.SmallTrees#canCall(n#0);
            assume _module.__default.SmallTrees($LS($LZ), n#0)
               == #_module.Stream.SCons($Box(_module.__default.SmallTree($LS($LZ), n#0 - 1)), 
                _module.__default.SmallTrees($LS($LZ), n#0));
            assume _module.__default.SmallTree#canCall(n#0 - 1)
               && _module.__default.SmallTrees#canCall(n#0);
            // CheckWellformedWithResult: any expression
            assume $Is(_module.__default.SmallTrees($LS($LZ), n#0), 
...
procedure {:_induction n#0} CheckWellformed$$_module.__default.Theorem(n#0: int where LitInt(0) <= n#0);
  free requires 10 == $FunctionContextHeight;
  modifies $Heap, $Tick;



procedure {:_induction n#0} Call$$_module.__default.Theorem(n#0: int where LitInt(0) <= n#0);
  modifies $Heap, $Tick;
  // user-defined postconditions
  free ensures _module.__default.SmallTree#canCall(n#0)
     && _module.__default.HasBoundedHeight#canCall(_module.__default.SmallTree($LS($LZ), n#0));
  free ensures _module.__default.HasBoundedHeight#canCall(_module.__default.SmallTree($LS($LZ), n#0))
     && 
    _module.__default.HasBoundedHeight(_module.__default.SmallTree($LS($LZ), n#0))
     && (exists n#1: int :: 
      { _module.__default.LowerThan($LS($LZ), _module.Tree.children(_module.__default.SmallTree($LS($LZ), n#0)), n#1) } 
      LitInt(0) <= n#1
         && 
        LitInt(0) <= n#1
         && _module.__default.LowerThan($LS($LZ), _module.Tree.children(_module.__default.SmallTree($LS($LZ), n#0)), n#1));
...
procedure {:_induction n#0} Impl$$_module.__default.Theorem(n#0: int where LitInt(0) <= n#0) returns ($_reverifyPost: bool);
  free requires 10 == $FunctionContextHeight;
  modifies $Heap, $Tick;
  // user-defined postconditions
  free ensures _module.__default.SmallTree#canCall(n#0)
     && _module.__default.HasBoundedHeight#canCall(_module.__default.SmallTree($LS($LZ), n#0));
  ensures _module.__default.HasBoundedHeight#canCall(_module.__default.SmallTree($LS($LZ), n#0))
     ==> _module.__default.HasBoundedHeight(_module.__default.SmallTree($LS($LZ), n#0))
       || (exists n#2: int :: 
        { _module.__default.LowerThan($LS($LZ), _module.Tree.children(_module.__default.SmallTree($LS($LZ), n#0)), n#2) } 
        LitInt(0) <= n#2
           && 
          LitInt(0) <= n#2
           && _module.__default.LowerThan($LS($LZ), _module.Tree.children(_module.__default.SmallTree($LS($LZ), n#0)), n#2));
...
implementation {:_induction n#0} Impl$$_module.__default.Theorem(n#0: int) returns ($_reverifyPost: bool)
{
  var $_Frame: <beta>[ref,Field beta]bool;
  ...
  var n##0: int;
    ...
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny3/WideTrees.dfy(44,0): initial state"} true;
    $initHeapForallStmt#0 := $Heap;
    havoc $Heap, $Tick;
    assume $initHeapForallStmt#0 == $Heap;
    assume (forall $ih#n0#0: int :: 
      LitInt(0) <= $ih#n0#0 && Lit(true) && 0 <= $ih#n0#0 && $ih#n0#0 < n#0
         ==> _module.__default.HasBoundedHeight(_module.__default.SmallTree($LS($LZ), $ih#n0#0)));
    $_reverifyPost := false;
    // ----- call statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny3/WideTrees.dfy(45,8)
    ...
    call Call$$_module.__default.Lemma(n##0);
    // TrCallStmt: After ProcessCallStmt
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny3/WideTrees.dfy(45,10)"} true;
}



procedure CheckWellformed$$_module.__default.Lemma(n#0: int where LitInt(0) <= n#0);
  free requires 6 == $FunctionContextHeight;
  modifies $Heap, $Tick;



procedure Call$$_module.__default.Lemma(n#0: int where LitInt(0) <= n#0);
  modifies $Heap, $Tick;
  // user-defined postconditions
  free ensures _module.__default.SmallTrees#canCall(n#0)
     && _module.__default.LowerThan#canCall(_module.__default.SmallTrees($LS($LZ), n#0), n#0);
  ensures _module.__default.LowerThan($LS($LS($LZ)), _module.__default.SmallTrees($LS($LS($LZ)), n#0), n#0);
  // frame condition
  free ensures old($Heap) == $Heap;



procedure {:_induction _k#0, n#1} CoCall$$_module.__default.Lemma_h(_k#0: Box, n#1: int where LitInt(0) <= n#1);
  modifies $Heap, $Tick;
  // user-defined postconditions
  free ensures _module.__default.SmallTrees#canCall(n#1)
     && _module.__default.LowerThan_h#canCall(_k#0, _module.__default.SmallTrees($LS($LZ), n#1), n#1);
  ensures _module.__default.LowerThan_h($LS($LS($LZ)), _k#0, _module.__default.SmallTrees($LS($LS($LZ)), n#1), n#1);
  // frame condition
  free ensures old($Heap) == $Heap;



procedure {:_induction _k#0, n#1} Impl$$_module.__default.Lemma_h(_k#0: Box, n#1: int where LitInt(0) <= n#1) returns ($_reverifyPost: bool);
...
implementation {:_induction _k#0, n#1} Impl$$_module.__default.Lemma_h(_k#0: Box, n#1: int) returns ($_reverifyPost: bool)
{
  var $_Frame: <beta>[ref,Field beta]bool;
  var $initHeapForallStmt#0: Heap;
  var _k##0: Box;
  ...
  var $initHeapForallStmt#1: Heap;
    ...
    // AddMethodImpl: Lemma#, Impl$$_module.__default.Lemma_h
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny3/WideTrees.dfy(47,15): initial state"} true;
    $initHeapForallStmt#0 := $Heap;
    havoc $Heap, $Tick;
    assume $initHeapForallStmt#0 == $Heap;
    assume (forall $ih#_k0#0: Box, $ih#n0#0: int :: 
      LitInt(0) <= $ih#n0#0
         ...
         ==> _module.__default.LowerThan_h($LS($LZ), $ih#_k0#0, _module.__default.SmallTrees($LS($LZ), $ih#n0#0), $ih#n0#0));
    $_reverifyPost := false;
    // ----- if statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny3/WideTrees.dfy(49,1)
    assume true;
    if (0 < ORD#Offset(_k#0))
    {
        // ----- if statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny3/WideTrees.dfy(50,3)
        assume true;
        if (0 < n#1)
        {
            // ----- call statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny3/WideTrees.dfy(51,10)
            // TrCallStmt: Before ProcessCallStmt
            ...
            assert (forall<alpha> $o: ref, $f: Field alpha :: false ==> $_Frame[$o, $f]);
            // ProcessCallStmt: Make the call
            call CoCall$$_module.__default.Lemma_h(_k##0, n##0);
            // TrCallStmt: After ProcessCallStmt
            assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny3/WideTrees.dfy(51,14)"} true;
        }
        else
        {
        }
    ...
    {
        // ----- forall statement (call) ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny3/WideTrees.dfy(47,16)
        $initHeapForallStmt#1 := $Heap;
        havoc $Heap, $Tick;
        assume $initHeapForallStmt#1 == $Heap;
        assume (forall _k'#0: Box, n#2: int :: 
          LitInt(0) <= n#2 && ORD#Less(_k'#0, _k#0)
             ==> _module.__default.LowerThan_h($LS($LZ), _k'#0, _module.__default.SmallTrees($LS($LZ), n#2), n#2));
        assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny3/WideTrees.dfy(47,15)"} true;
    }
}
