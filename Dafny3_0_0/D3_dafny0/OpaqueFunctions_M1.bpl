...
axiom 0 <= $FunctionContextHeight
   ==> (forall $ly: LayerType :: 
    { M1.__default.F($ly) } 
    M1.__default.F#canCall() || 0 != $FunctionContextHeight ==> true);
...
axiom 0 <= $FunctionContextHeight
   ==> (forall $ly: LayerType :: 
    { M1.__default.F($LS($ly)) } 
    M1.__default.F#canCall() || 0 != $FunctionContextHeight
       ==> M1.__default.F($LS($ly)) == LitInt(12));
...
procedure {:opaque} CheckWellformed$$M1.__default.F();
  free requires 0 == $FunctionContextHeight;
  modifies $Heap, $Tick;



procedure {:auto_generated} {:opaque_reveal} {:verify false} CheckWellformed$$M1.__default.reveal__F();
  free requires 1 == $FunctionContextHeight;
  modifies $Heap, $Tick;



const MoreFuel_M1._default.F0: LayerType;

procedure {:auto_generated} {:opaque_reveal} {:verify false} Call$$M1.__default.reveal__F();
  modifies $Heap, $Tick;
  // frame condition
  free ensures old($Heap) == $Heap;
  free ensures StartFuel_M1._default.F == $LS(MoreFuel_M1._default.F0);
  free ensures StartFuelAssert_M1._default.F == $LS($LS(MoreFuel_M1._default.F0));
  // Shortcut to LZ
  free ensures AsFuelBottom(MoreFuel_M1._default.F0) == MoreFuel_M1._default.F0;
