...
procedure CheckWellformed$$ModuleWithIterator.Iter.__ctor(this: ref
       where this != null
         && 
...
procedure Call$$ModuleWithIterator.Iter.__ctor(x#0: int)
   returns (this: ref
       where this != null
         && 
        $Is(this, Tclass.ModuleWithIterator.Iter())
  ...
  free ensures true;
  ensures ModuleWithIterator.Iter.x(this) == x#0;
  free ensures true;
  ensures Seq#Equal(read($Heap, this, ModuleWithIterator.Iter.ys), Seq#Empty(): Seq Box);
  free ensures ModuleWithIterator.Iter.Valid#canCall($Heap, this);
  ensures ModuleWithIterator.Iter.Valid($Heap, this);
  free ensures true;
  ensures Set#Equal(ModuleWithIterator.Iter.__reads(this), Set#Empty(): Set Box);
  free ensures true;
  ensures Set#Equal(ModuleWithIterator.Iter.__modifies(this), Set#Empty(): Set Box);
  free ensures true;
  ensures Set#Equal(read($Heap, this, ModuleWithIterator.Iter.__new), Set#Empty(): Set Box);
  free ensures true;
  ensures ModuleWithIterator.Iter.__decreases0(this) == x#0;
...
procedure CheckWellformed$$ModuleWithIterator.Iter.Valid(this: ref
       where this != null
         && 
        $Is(this, Tclass.ModuleWithIterator.Iter())
         && $IsAlloc(this, Tclass.ModuleWithIterator.Iter(), $Heap));
...
implementation CheckWellformed$$ModuleWithIterator.Iter.Valid(this: ref)
{
  ...
  var b$reqreads#0: bool;
    ...
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Iterators.dfy(388,11): initial state"} true;
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc)
         ==> $o == this
           || ModuleWithIterator.Iter.__reads(this)[$Box($o)]
           || read($Heap, this, ModuleWithIterator.Iter.__new)[$Box($o)]);
    b$reqreads#0 := $_Frame[this, ModuleWithIterator.Iter.__new];
    assert b$reqreads#0;
    if (*)
    {
        assume false;
...
procedure Call$$ModuleWithIterator.Iter.MoveNext(this: ref
       where this != null
         && 
        $Is(this, Tclass.ModuleWithIterator.Iter())
         && $IsAlloc(this, Tclass.ModuleWithIterator.Iter(), $Heap))
  ...
  requires ModuleWithIterator.Iter.Valid($Heap, this);
  modifies $Heap, $Tick;
  // user-defined postconditions
  free ensures true;
  ensures (forall $o: ref :: 
    ...
    read($Heap, this, ModuleWithIterator.Iter.__new)[$Box($o)]
         && !read(old($Heap), this, ModuleWithIterator.Iter.__new)[$Box($o)]
       ==> $o != null && !read(old($Heap), $o, alloc));
  free ensures true;
  ensures !read($Heap, this, ModuleWithIterator.Iter.__new)[$Box(null)];
  free ensures more#0 ==> ModuleWithIterator.Iter.Valid#canCall($Heap, this);
  ensures more#0 ==> ModuleWithIterator.Iter.Valid($Heap, this);
  free ensures true;
  ensures Seq#Equal(read($Heap, this, ModuleWithIterator.Iter.ys), 
    (if more#0
       then Seq#Append(read(old($Heap), this, ModuleWithIterator.Iter.ys), 
        Seq#Build(Seq#Empty(): Seq Box, $Box(read($Heap, this, ModuleWithIterator.Iter.y))))
       else read(old($Heap), this, ModuleWithIterator.Iter.ys)));
  // frame condition: object granularity
  free ensures (forall $o: ref :: 
    { $Heap[$o] } 
    $o != null && read(old($Heap), $o, alloc)
       ==> $Heap[$o] == old($Heap)[$o]
         || 
        $o == this
         || ModuleWithIterator.Iter.__modifies(this)[$Box($o)]
         || read(old($Heap), this, ModuleWithIterator.Iter.__new)[$Box($o)]);
  // boilerplate
  free ensures $HeapSucc(old($Heap), $Heap);
...
procedure Impl$$ModuleWithIterator.Iter(this: ref
       where this != null
         && 
        $Is(this, Tclass.ModuleWithIterator.Iter())
         && $IsAlloc(this, Tclass.ModuleWithIterator.Iter(), $Heap), 
...
// Box/unbox axiom for Tclass.ModuleWithIterator.__default
axiom (forall bx: Box :: 
  { $IsBox(bx, Tclass.ModuleWithIterator.__default()) } 
  $IsBox(bx, Tclass.ModuleWithIterator.__default())
     ==> $Box($Unbox(bx): ref) == bx
       && $Is($Unbox(bx): ref, Tclass.ModuleWithIterator.__default()));
