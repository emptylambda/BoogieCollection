...
axiom 2 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, n#0: int :: 
    { CoRecursion.__default.AscendingChain($ly, n#0) } 
    CoRecursion.__default.AscendingChain#canCall(n#0) || 2 != $FunctionContextHeight
...
procedure CheckWellformed$$CoRecursion.__default.AscendingChain(n#0: int);
  free requires 2 == $FunctionContextHeight;
  modifies $Heap, $Tick;



implementation CheckWellformed$$CoRecursion.__default.AscendingChain(n#0: int)
{
  ...
  var b$reqreads#0: bool;
    ...
    // AddWellformednessCheck for function AscendingChain
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(25,18): initial state"} true;
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    if (*)
    {
        assume $Is(CoRecursion.__default.AscendingChain($LS($LZ), n#0), 
          Tclass.CoRecursion.Stream(TInt));
        assume false;
    }
    else
    {
        ...
        assume CoRecursion.__default.AscendingChain#canCall(n#0 + 1);
        assume CoRecursion.Stream.More_q(CoRecursion.__default.AscendingChain($LS($LZ), n#0 + 1));
        assume CoRecursion.__default.AscendingChain($LS($LZ), n#0)
           == #CoRecursion.Stream.More($Box(n#0), CoRecursion.__default.AscendingChain($LS($LZ), n#0 + 1));
        assume CoRecursion.__default.AscendingChain#canCall(n#0 + 1);
...
procedure CheckWellformed$$CoRecursion.__default.Prefix(CoRecursion._default.Prefix$_T0: Ty, 
    n#0: int where LitInt(0) <= n#0, 
    s#0: DatatypeType
       where $Is(s#0, Tclass.CoRecursion.Stream(CoRecursion._default.Prefix$_T0)));
  free requires 4 == $FunctionContextHeight;
  modifies $Heap, $Tick;



implementation CheckWellformed$$CoRecursion.__default.Prefix(CoRecursion._default.Prefix$_T0: Ty, n#0: int, s#0: DatatypeType)
{
  var $_Frame: <beta>[ref,Field beta]bool;
  var ##n#0: int;
  ...
  var b$reqreads#0: bool;
    ...
    // AddWellformednessCheck for function Prefix
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(32,18): initial state"} true;
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    if (*)
    {
        assume $Is(CoRecursion.__default.Prefix(CoRecursion._default.Prefix$_T0, $LS($LZ), n#0, s#0), 
          Tclass.CoRecursion.List(CoRecursion._default.Prefix$_T0));
        assume false;
    }
    else
    {
        ...
        if (n#0 == LitInt(0))
        {
            assume CoRecursion.__default.Prefix(CoRecursion._default.Prefix$_T0, $LS($LZ), n#0, s#0)
               == Lit(#CoRecursion.List.Nil());
            assume true;
            // CheckWellformedWithResult: any expression
            assume $Is(CoRecursion.__default.Prefix(CoRecursion._default.Prefix$_T0, $LS($LZ), n#0, s#0), 
              Tclass.CoRecursion.List(CoRecursion._default.Prefix$_T0));
        ...
        {
            assume CoRecursion.Stream.More_q(s#0);
            assume CoRecursion.Stream.More_q(s#0);
            assert $Is(n#0 - 1, Tclass._System.nat());
            ##n#0 := n#0 - 1;
            // assume allocatedness for argument to function
            assume $IsAlloc(##n#0, Tclass._System.nat(), $Heap);
            ##s#0 := CoRecursion.Stream.rest(s#0);
            // assume allocatedness for argument to function
            assume $IsAlloc(##s#0, Tclass.CoRecursion.Stream(CoRecursion._default.Prefix$_T0), $Heap);
            b$reqreads#0 := (forall<alpha> $o: ref, $f: Field alpha :: false ==> $_Frame[$o, $f]);
            assert 0 <= n#0 || ##n#0 == n#0;
            assert ##n#0 < n#0;
            assume CoRecursion.__default.Prefix#canCall(CoRecursion._default.Prefix$_T0, n#0 - 1, CoRecursion.Stream.rest(s#0));
            assume CoRecursion.__default.Prefix(CoRecursion._default.Prefix$_T0, $LS($LZ), n#0, s#0)
               == #CoRecursion.List.Cons(CoRecursion.Stream.head(s#0), 
                CoRecursion.__default.Prefix(CoRecursion._default.Prefix$_T0, $LS($LZ), n#0 - 1, CoRecursion.Stream.rest(s#0)));
            assume CoRecursion.Stream.More_q(s#0)
               && 
              CoRecursion.Stream.More_q(s#0)
               && CoRecursion.__default.Prefix#canCall(CoRecursion._default.Prefix$_T0, n#0 - 1, CoRecursion.Stream.rest(s#0));
...
procedure Impl$$CoRecursion.__default.TestMain() returns ($_reverifyPost: bool);
  free requires 8 == $FunctionContextHeight;
  modifies $Heap, $Tick;
  // frame condition: object granularity
  free ensures (forall $o: ref :: 
...
implementation Impl$$CoRecursion.__default.TestMain() returns ($_reverifyPost: bool)
{
  var $_Frame: <beta>[ref,Field beta]bool;
  var m#0: int;
  var cell#0: ref
     where $Is(cell#0, Tclass.CoRecursion.Cell())
       && $IsAlloc(cell#0, Tclass.CoRecursion.Cell(), $Heap);
  var $nw: ref;
  var $rhs#0: int;
  var mr#0: DatatypeType
     where $Is(mr#0, Tclass.CoRecursion.Stream(TInt))
       && $IsAlloc(mr#0, Tclass.CoRecursion.Stream(TInt), $Heap);
  var ##n#0: int;
  var $rhs#1: int;
  var l#0: DatatypeType
     where $Is(l#0, Tclass.CoRecursion.List(TInt))
       && $IsAlloc(l#0, Tclass.CoRecursion.List(TInt), $Heap);
  var ##n#1: int;
  var ##s#0: DatatypeType;
  ...
  var hi##1: int;
    ...
    // AddMethodImpl: TestMain, Impl$$CoRecursion.__default.TestMain
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(48,20): initial state"} true;
    $_reverifyPost := false;
    // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(49,11)
    assume true;
    assume true;
    m#0 := LitInt(17);
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(49,15)"} true;
    // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(50,14)
    assume true;
    havoc $nw;
    assume $nw != null && dtype($nw) == Tclass.CoRecursion.Cell?();
    assume !read($Heap, $nw, alloc);
    $Heap := update($Heap, $nw, alloc, true);
    assume $IsGoodHeap($Heap);
    assume $IsHeapAnchor($Heap);
    cell#0 := $nw;
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(50,24)"} true;
    // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(51,15)
    assert cell#0 != null;
    assume true;
    assert $_Frame[cell#0, CoRecursion.Cell.data];
    assume true;
    $rhs#0 := LitInt(40);
    $Heap := update($Heap, cell#0, CoRecursion.Cell.data, $rhs#0);
    assume $IsGoodHeap($Heap);
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(51,19)"} true;
    // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(52,12)
    assume true;
    assert cell#0 != null;
    ##n#0 := read($Heap, cell#0, CoRecursion.Cell.data);
    // assume allocatedness for argument to function
    assume $IsAlloc(##n#0, TInt, $Heap);
    assume CoRecursion.__default.AscendingChain#canCall(read($Heap, cell#0, CoRecursion.Cell.data));
    assume CoRecursion.Stream.More_q(CoRecursion.__default.AscendingChain($LS($LZ), read($Heap, cell#0, CoRecursion.Cell.data)));
    assume CoRecursion.__default.AscendingChain#canCall(read($Heap, cell#0, CoRecursion.Cell.data));
    mr#0 := #CoRecursion.Stream.More($Box(LitInt(400)), 
      #CoRecursion.Stream.More($Box(LitInt(320)), 
        CoRecursion.__default.AscendingChain($LS($LZ), read($Heap, cell#0, CoRecursion.Cell.data))));
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(52,61)"} true;
    // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(53,7)
    assume true;
    assume true;
    m#0 := LitInt(30);
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(53,11)"} true;
    // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(54,15)
    assert cell#0 != null;
    assume true;
    assert $_Frame[cell#0, CoRecursion.Cell.data];
    assume true;
    $rhs#1 := LitInt(60);
    $Heap := update($Heap, cell#0, CoRecursion.Cell.data, $rhs#1);
    assume $IsGoodHeap($Heap);
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(54,19)"} true;
    // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(55,11)
    assume true;
    ...
    ##s#0 := mr#0;
    // assume allocatedness for argument to function
    assume $IsAlloc(##s#0, Tclass.CoRecursion.Stream(TInt), $Heap);
    assume CoRecursion.__default.Prefix#canCall(TInt, LitInt(5), mr#0);
    assume CoRecursion.__default.Prefix#canCall(TInt, LitInt(5), mr#0);
    l#0 := CoRecursion.__default.Prefix(TInt, $LS($LZ), LitInt(5), mr#0);
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(55,26)"} true;
    // ----- while statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(56,5)
    ...
    {
        assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(56,4): after some loop iterations"} true;
        if (!$w$loop#0)
        {
            assume true;
            assume false;
        ...
        assume $IsA#CoRecursion.List(l#0);
        if (CoRecursion.List#Equal(l#0, #CoRecursion.List.Nil()))
        {
            break;
        }
        ...
        if (l#0 == #CoRecursion.List.Cons($Box(_mcc#0#0_0_0), _mcc#1#0_0_0))
        {
            assume $Is(_mcc#1#0_0_0, Tclass.CoRecursion.List(TInt))
               && $IsAlloc(_mcc#1#0_0_0, Tclass.CoRecursion.List(TInt), $Heap);
            havoc y#0_0_0;
            assume $Is(y#0_0_0, Tclass.CoRecursion.List(TInt))
               && $IsAlloc(y#0_0_0, Tclass.CoRecursion.List(TInt), $Heap);
            assume let#0_0_0#0#0 == _mcc#1#0_0_0;
            assume true;
            // CheckWellformedWithResult: any expression
            assume $Is(let#0_0_0#0#0, Tclass.CoRecursion.List(TInt));
        ...
        else if (l#0 == #CoRecursion.List.Nil())
        {
            assert false;
        }
        else
        ...
        // ----- print statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(60,7)
        assert CoRecursion.List.Cons_q(l#0);
        assume true;
        assume true;
        // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(61,9)
        assume true;
        assert CoRecursion.List.Cons_q(l#0);
        assume true;
        l#0 := CoRecursion.List.cdr(l#0);
        assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(61,16)"} true;
        // ----- loop termination check ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(56,5)
        assert DtRank(l#0) < DtRank($decr$loop#00);
    }

    // ----- call statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(63,23)
    assume true;
    ...
    call $rhs##0 := Call$$CoRecursion.__default.OneLess(lo##0, hi##0);
    // TrCallStmt: After ProcessCallStmt
    nio#0 := $rhs##0;
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(63,29)"} true;
    // ----- print statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(64,5)
    assume true;
    assume true;
    // ----- call statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(65,20)
    assume true;
    // TrCallStmt: Adding lhs with type int
    // TrCallStmt: Before ProcessCallStmt
    assume true;
    ...
    assert (forall<alpha> $o: ref, $f: Field alpha :: false ==> $_Frame[$o, $f]);
    // ProcessCallStmt: Make the call
    call $rhs##1 := Call$$CoRecursion.__default.OneLess_k(lo##1, hi##1);
    // TrCallStmt: After ProcessCallStmt
    nio#0 := $rhs##1;
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(65,26)"} true;
    // ----- print statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(66,5)
    assume true;
    assume true;
}
...
procedure CheckWellformed$$CoRecursion.__default.OneLess(lo#0: int, hi#0: int) returns (m#0: int);
  free requires 6 == $FunctionContextHeight;
  modifies $Heap, $Tick;



procedure Call$$CoRecursion.__default.OneLess(lo#0: int, hi#0: int) returns (m#0: int);
...
procedure Impl$$CoRecursion.__default.OneLess(lo#0: int, hi#0: int) returns (m#0: int, $_reverifyPost: bool);
  free requires 6 == $FunctionContextHeight;
  // user-defined preconditions
  requires lo#0 < hi#0;
  modifies $Heap, $Tick;
...
implementation Impl$$CoRecursion.__default.OneLess(lo#0: int, hi#0: int) returns (m#0: int, $_reverifyPost: bool)
{
  var $_Frame: <beta>[ref,Field beta]bool;
  var eg$y#0: int;
  var y#0_0: int;
  ...
  var hi##0_0: int;
    ...
    // AddMethodImpl: OneLess, Impl$$CoRecursion.__default.OneLess
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(73,2): initial state"} true;
    $_reverifyPost := false;
    // ----- if statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(74,5)
    ...
    if (*)
    {
        assume lo#0 < y#0_0 && y#0_0 < hi#0;
        // ----- call statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(75,19)
        assume true;
        // TrCallStmt: Adding lhs with type int
        // TrCallStmt: Before ProcessCallStmt
        assume true;
        ...
        hi##0_0 := hi#0;
        assert (forall<alpha> $o: ref, $f: Field alpha :: false ==> $_Frame[$o, $f]);
        assert 0 <= hi#0 - lo#0 || hi##0_0 - lo##0_0 == hi#0 - lo#0;
        assert hi##0_0 - lo##0_0 < hi#0 - lo#0;
        // ProcessCallStmt: Make the call
        call $rhs##0_0 := Call$$CoRecursion.__default.OneLess(lo##0_0, hi##0_0);
        // TrCallStmt: After ProcessCallStmt
        m#0 := $rhs##0_0;
        assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(75,25)"} true;
    }
    else
    {
        assume !(exists eg$y#1: int :: lo#0 < eg$y#1 && eg$y#1 < hi#0);
        // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(77,9)
        assume true;
        assume true;
        m#0 := lo#0;
        assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(77,13)"} true;
    }
}



procedure CheckWellformed$$CoRecursion.__default.OneLess_k(lo#0: int, hi#0: int) returns (m#0: int);
  free requires 7 == $FunctionContextHeight;
  modifies $Heap, $Tick;



procedure Call$$CoRecursion.__default.OneLess_k(lo#0: int, hi#0: int) returns (m#0: int);
  // user-defined preconditions
  requires lo#0 < hi#0;
  modifies $Heap, $Tick;
  // frame condition: object granularity
...
procedure Impl$$CoRecursion.__default.OneLess_k(lo#0: int, hi#0: int) returns (m#0: int, $_reverifyPost: bool);
  free requires 7 == $FunctionContextHeight;
  // user-defined preconditions
  requires lo#0 < hi#0;
  modifies $Heap, $Tick;
...
implementation Impl$$CoRecursion.__default.OneLess_k(lo#0: int, hi#0: int) returns (m#0: int, $_reverifyPost: bool)
{
  var $_Frame: <beta>[ref,Field beta]bool;
  var $rhs##1_0: int;
  var lo##1_0: int;
  ...
  var hi##2_0: int;
    ...
    // AddMethodImpl: OneLess', Impl$$CoRecursion.__default.OneLess_k
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(85,2): initial state"} true;
    $_reverifyPost := false;
    // ----- alternative statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(86,5)
    havoc y#2_0;
    if (*)
    {
        // Begin Comprehension WF check
        ...
        // End Comprehension WF check
        assume true;
        assume lo#0 < y#2_0 && y#2_0 < hi#0;
        // ----- call statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(88,22)
        assume true;
        // TrCallStmt: Adding lhs with type int
        // TrCallStmt: Before ProcessCallStmt
        assume true;
        ...
        hi##2_0 := hi#0;
        assert (forall<alpha> $o: ref, $f: Field alpha :: false ==> $_Frame[$o, $f]);
        assert 0 <= hi#0 - lo#0 || hi##2_0 - lo##2_0 == hi#0 - lo#0;
        assert hi##2_0 - lo##2_0 < hi#0 - lo#0;
        // ProcessCallStmt: Make the call
        call $rhs##2_0 := Call$$CoRecursion.__default.OneLess_k(lo##2_0, hi##2_0);
        // TrCallStmt: After ProcessCallStmt
        m#0 := $rhs##2_0;
        assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(88,28)"} true;
    }
    else if (*)
    {
        assume true;
        assume lo#0 + 1 < hi#0;
        // ----- call statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(90,22)
        assume true;
        // TrCallStmt: Adding lhs with type int
        // TrCallStmt: Before ProcessCallStmt
        assume true;
        ...
        hi##1_0 := hi#0;
        assert (forall<alpha> $o: ref, $f: Field alpha :: false ==> $_Frame[$o, $f]);
        assert 0 <= hi#0 - lo#0 || hi##1_0 - lo##1_0 == hi#0 - lo#0;
        assert hi##1_0 - lo##1_0 < hi#0 - lo#0;
        // ProcessCallStmt: Make the call
        call $rhs##1_0 := Call$$CoRecursion.__default.OneLess_k(lo##1_0, hi##1_0);
        // TrCallStmt: After ProcessCallStmt
        m#0 := $rhs##1_0;
        assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(90,31)"} true;
    }
    else if (*)
    {
        assume true;
        assume lo#0 + 1 == hi#0;
        // ----- assignment statement ----- /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(92,11)
        assume true;
        assume true;
        m#0 := lo#0;
        assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Compilation.dfy(92,15)"} true;
    }
    else
    {
        assume true;
        assume true;
        assume true;
        assume !(exists eg$y#0: int :: lo#0 < eg$y#0 && eg$y#0 < hi#0)
           && hi#0 <= lo#0 + 1
           && lo#0 + 1 != hi#0;
        assert false;
    }
