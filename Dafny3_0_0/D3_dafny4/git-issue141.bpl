// Dafny 3.0.0.30204
// Command Line Options: -compile:0 -noVerify /Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny4/git-issue141.dfy -print:./git-issue141.bpl
...
axiom FDim(_module.List.spine) == 0
   && FieldOfDecl(class._module.List?, field$spine) == _module.List.spine
   && $IsGhostField(_module.List.spine);

const _module.List.spine: Field (Seq Box);

// List.spine: Type axiom
axiom (forall _module.List$T: Ty, $h: Heap, $o: ref :: 
  { read($h, $o, _module.List.spine), Tclass._module.List?(_module.List$T) } 
  $IsGoodHeap($h)
       && 
      $o != null
       && dtype($o) == Tclass._module.List?(_module.List$T)
     ==> $Is(read($h, $o, _module.List.spine), TSeq(Tclass._module.Node(_module.List$T))));

// List.spine: Allocation axiom
axiom (forall _module.List$T: Ty, $h: Heap, $o: ref :: 
  { read($h, $o, _module.List.spine), Tclass._module.List?(_module.List$T) } 
  $IsGoodHeap($h)
       && 
      $o != null
       && dtype($o) == Tclass._module.List?(_module.List$T)
       && read($h, $o, alloc)
     ==> $IsAlloc(read($h, $o, _module.List.spine), TSeq(Tclass._module.Node(_module.List$T)), $h));
    ...
    _module.List.Valid#canCall(_module.List$T, $Heap, this)
       ...
       ==> _module.List.Valid(_module.List$T, $Heap, this)
         == (true
           && 
          (forall i#0: int :: 
            { $Unbox(Seq#Index(read($Heap, this, _module.List.spine), i#0)): ref } 
            LitInt(0) <= i#0 && i#0 < Seq#Length(read($Heap, this, _module.List.spine))
               ==> read($Heap, this, _module.List.repr)[Seq#Index(read($Heap, this, _module.List.spine), i#0)])
           && (forall i#1: int, _t#0#0: int :: 
            { $Unbox(Seq#Index(read($Heap, this, _module.List.spine), _t#0#0)): ref, $Unbox(Seq#Index(read($Heap, this, _module.List.spine), i#1)): ref } 
            _t#0#0 == i#1 + 1
                 && 
                LitInt(0) <= i#1
                 && i#1 < Seq#Length(read($Heap, this, _module.List.spine))
               ==> read($Heap, 
                  $Unbox(Seq#Index(read($Heap, this, _module.List.spine), i#1)): ref, 
                  _module.Node.next)
                 == (if i#1 < Seq#Length(read($Heap, this, _module.List.spine)) - 1
                   then $Unbox(Seq#Index(read($Heap, this, _module.List.spine), _t#0#0)): ref
                   else null))));
...
implementation CheckWellformed$$_module.List.Valid(_module.List$T: Ty, this: ref)
{
  ...
  var b$reqreads#7: bool;
    ...
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny4/git-issue141.dfy(12,14): initial state"} true;
    ...
    else
    {
        ...
        if (Lit(true))
        {
            ...
            if (true)
            {
                if (LitInt(0) <= i#2)
                {
                    b$reqreads#1 := $_Frame[this, _module.List.spine];
                }

                if (LitInt(0) <= i#2 && i#2 < Seq#Length(read($Heap, this, _module.List.spine)))
                {
                    b$reqreads#2 := $_Frame[this, _module.List.spine];
                    assert 0 <= i#2 && i#2 < Seq#Length(read($Heap, this, _module.List.spine));
                    b$reqreads#3 := $_Frame[this, _module.List.repr];
                    if (read($Heap, this, _module.List.repr)[Seq#Index(read($Heap, this, _module.List.spine), i#2)])
                    {
                        b$reqreads#4 := $_Frame[this, _module.List.spine];
                        assert 0 <= i#2 && i#2 < Seq#Length(read($Heap, this, _module.List.spine));
                        assert $Unbox(Seq#Index(read($Heap, this, _module.List.spine), i#2)): ref != null;
                        b$reqreads#5 := $_Frame[$Unbox(Seq#Index(read($Heap, this, _module.List.spine), i#2)): ref, _module.Node.next];
                        b$reqreads#6 := $_Frame[this, _module.List.spine];
                        if (i#2 < Seq#Length(read($Heap, this, _module.List.spine)) - 1)
                        {
                            b$reqreads#7 := $_Frame[this, _module.List.spine];
                            assert 0 <= i#2 + 1 && i#2 + 1 < Seq#Length(read($Heap, this, _module.List.spine));
                        }
                        else
                        {
                        }
        ...
        assume _module.List.Valid(_module.List$T, $Heap, this)
           == (true
             && 
            (forall i#3: int :: 
              { $Unbox(Seq#Index(read($Heap, this, _module.List.spine), i#3)): ref } 
              LitInt(0) <= i#3 && i#3 < Seq#Length(read($Heap, this, _module.List.spine))
                 ==> read($Heap, this, _module.List.repr)[Seq#Index(read($Heap, this, _module.List.spine), i#3)])
             && (forall i#3: int, _t#0#1: int :: 
              { $Unbox(Seq#Index(read($Heap, this, _module.List.spine), _t#0#1)): ref, $Unbox(Seq#Index(read($Heap, this, _module.List.spine), i#3)): ref } 
              _t#0#1 == i#3 + 1
                   && 
                  LitInt(0) <= i#3
                   && i#3 < Seq#Length(read($Heap, this, _module.List.spine))
                 ==> read($Heap, 
                    $Unbox(Seq#Index(read($Heap, this, _module.List.spine), i#3)): ref, 
                    _module.Node.next)
                   == (if i#3 < Seq#Length(read($Heap, this, _module.List.spine)) - 1
                     then $Unbox(Seq#Index(read($Heap, this, _module.List.spine), _t#0#1)): ref
                     else null)));
        assume true;
