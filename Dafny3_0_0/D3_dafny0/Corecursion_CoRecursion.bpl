...
axiom (forall d: DatatypeType, CoRecursion.List$T: Ty, $h: Heap :: 
  { $IsAllocBox(CoRecursion.List._h0(d), CoRecursion.List$T, $h) } 
  $IsGoodHeap($h)
       && 
      CoRecursion.List.Cons_q(d)
...
// Destructor $IsAlloc
axiom (forall d: DatatypeType, CoRecursion.List$T: Ty, $h: Heap :: 
  { $IsAlloc(CoRecursion.List._h1(d), Tclass.CoRecursion.List(CoRecursion.List$T), $h) } 
  $IsGoodHeap($h)
       && 
      CoRecursion.List.Cons_q(d)
       && $IsAlloc(d, Tclass.CoRecursion.List(CoRecursion.List$T), $h)
...
function CoRecursion.List._h0(DatatypeType) : Box;

// Constructor injectivity
axiom (forall a#30#0#0: Box, a#30#1#0: DatatypeType :: 
  { #CoRecursion.List.Cons(a#30#0#0, a#30#1#0) } 
  CoRecursion.List._h0(#CoRecursion.List.Cons(a#30#0#0, a#30#1#0)) == a#30#0#0);
...
function CoRecursion.List._h1(DatatypeType) : DatatypeType;

// Constructor injectivity
axiom (forall a#32#0#0: Box, a#32#1#0: DatatypeType :: 
  { #CoRecursion.List.Cons(a#32#0#0, a#32#1#0) } 
  CoRecursion.List._h1(#CoRecursion.List.Cons(a#32#0#0, a#32#1#0)) == a#32#1#0);
...
axiom (forall a: DatatypeType, b: DatatypeType :: 
  ...
  CoRecursion.List.Cons_q(a) && CoRecursion.List.Cons_q(b)
     ==> (CoRecursion.List#Equal(a, b)
       <==> CoRecursion.List._h0(a) == CoRecursion.List._h0(b)
         && CoRecursion.List#Equal(CoRecursion.List._h1(a), CoRecursion.List._h1(b))));
...
implementation CheckWellformed$$CoRecursion.__default.AscendingChain(n#0: int)
{
  ...
  var b$reqreads#0: bool;
    ...
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Corecursion.dfy(9,11): initial state"} true;
...
// function declaration for CoRecursion._default.AscendingChainAndRead
function CoRecursion.__default.AscendingChainAndRead($ly: LayerType, $heap: Heap, n#0: int) : DatatypeType;

function CoRecursion.__default.AscendingChainAndRead#canCall($heap: Heap, n#0: int) : bool;

// layer synonym axiom
axiom (forall $ly: LayerType, $Heap: Heap, n#0: int :: 
  { CoRecursion.__default.AscendingChainAndRead($LS($ly), $Heap, n#0) } 
  CoRecursion.__default.AscendingChainAndRead($LS($ly), $Heap, n#0)
     == CoRecursion.__default.AscendingChainAndRead($ly, $Heap, n#0));

// fuel synonym axiom
axiom (forall $ly: LayerType, $Heap: Heap, n#0: int :: 
  { CoRecursion.__default.AscendingChainAndRead(AsFuelBottom($ly), $Heap, n#0) } 
  CoRecursion.__default.AscendingChainAndRead($ly, $Heap, n#0)
     == CoRecursion.__default.AscendingChainAndRead($LZ, $Heap, n#0));

// frame axiom for CoRecursion.__default.AscendingChainAndRead
axiom (forall $ly: LayerType, $h0: Heap, $h1: Heap, n#0: int :: 
  { $IsHeapAnchor($h0), $HeapSucc($h0, $h1), CoRecursion.__default.AscendingChainAndRead($ly, $h1, n#0) } 
  $IsGoodHeap($h0)
       && $IsGoodHeap($h1)
       && (CoRecursion.__default.AscendingChainAndRead#canCall($h0, n#0)
         || LitInt(0) <= n#0)
       && 
      $IsHeapAnchor($h0)
       && $HeapSucc($h0, $h1)
     ==> 
    (forall<alpha> $o: ref, $f: Field alpha :: 
      $o != null && $o == null ==> read($h0, $o, $f) == read($h1, $o, $f))
     ==> CoRecursion.__default.AscendingChainAndRead($ly, $h0, n#0)
       == CoRecursion.__default.AscendingChainAndRead($ly, $h1, n#0));

// consequence axiom for CoRecursion.__default.AscendingChainAndRead
axiom 4 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, $Heap: Heap, n#0: int :: 
    { CoRecursion.__default.AscendingChainAndRead($ly, $Heap, n#0) } 
    CoRecursion.__default.AscendingChainAndRead#canCall($Heap, n#0)
         || (4 != $FunctionContextHeight && $IsGoodHeap($Heap) && LitInt(0) <= n#0)
       ==> $Is(CoRecursion.__default.AscendingChainAndRead($ly, $Heap, n#0), 
        Tclass.CoRecursion.Stream(TInt)));

function CoRecursion.__default.AscendingChainAndRead#requires(LayerType, Heap, int) : bool;

// #requires axiom for CoRecursion.__default.AscendingChainAndRead
axiom (forall $ly: LayerType, $Heap: Heap, n#0: int :: 
  { CoRecursion.__default.AscendingChainAndRead#requires($ly, $Heap, n#0), $IsGoodHeap($Heap) } 
  $IsGoodHeap($Heap) && LitInt(0) <= n#0
     ==> CoRecursion.__default.AscendingChainAndRead#requires($ly, $Heap, n#0) == true);

// definition axiom for CoRecursion.__default.AscendingChainAndRead (revealed)
axiom 4 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, $Heap: Heap, n#0: int :: 
    { CoRecursion.__default.AscendingChainAndRead($LS($ly), $Heap, n#0), $IsGoodHeap($Heap) } 
    CoRecursion.__default.AscendingChainAndRead#canCall($Heap, n#0)
         || (4 != $FunctionContextHeight && $IsGoodHeap($Heap) && LitInt(0) <= n#0)
       ==> CoRecursion.__default.AscendingChainAndRead#canCall($Heap, n#0 + 1)
         && CoRecursion.__default.AscendingChainAndRead($LS($ly), $Heap, n#0)
           == #CoRecursion.Stream.More($Box(n#0), CoRecursion.__default.AscendingChainAndRead($ly, $Heap, n#0 + 1)));

procedure CheckWellformed$$CoRecursion.__default.AscendingChainAndRead(n#0: int where LitInt(0) <= n#0);
  free requires 4 == $FunctionContextHeight;
  modifies $Heap, $Tick;



implementation CheckWellformed$$CoRecursion.__default.AscendingChainAndRead(n#0: int)
{
  var $_Frame: <beta>[ref,Field beta]bool;
  var ##n#0: int;
  var b$reqreads#0: bool;
    ...
    // AddWellformednessCheck for function AscendingChainAndRead
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Corecursion.dfy(14,11): initial state"} true;
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> $o == null);
    if (*)
    {
        assume $Is(CoRecursion.__default.AscendingChainAndRead($LS($LZ), $Heap, n#0), 
          Tclass.CoRecursion.Stream(TInt));
        assume false;
    }
    else
    {
        ...
        b$reqreads#0 := (forall<alpha> $o: ref, $f: Field alpha :: 
          $o != null && read($Heap, $o, alloc) && $o == null ==> $_Frame[$o, $f]);
        assert 0 <= n#0
           || (Set#Subset(Set#UnionOne(Set#Empty(): Set Box, $Box(null)), 
              Set#UnionOne(Set#Empty(): Set Box, $Box(null)))
             && !Set#Subset(Set#UnionOne(Set#Empty(): Set Box, $Box(null)), 
              Set#UnionOne(Set#Empty(): Set Box, $Box(null))))
           || ##n#0 == n#0;
        assert (Set#Subset(Set#UnionOne(Set#Empty(): Set Box, $Box(null)), 
              Set#UnionOne(Set#Empty(): Set Box, $Box(null)))
             && !Set#Subset(Set#UnionOne(Set#Empty(): Set Box, $Box(null)), 
              Set#UnionOne(Set#Empty(): Set Box, $Box(null))))
           || (Set#Equal(Set#UnionOne(Set#Empty(): Set Box, $Box(null)), 
              Set#UnionOne(Set#Empty(): Set Box, $Box(null)))
             && ##n#0 < n#0);
        assume CoRecursion.__default.AscendingChainAndRead#canCall($Heap, n#0 + 1);
        assume CoRecursion.Stream.More_q(CoRecursion.__default.AscendingChainAndRead($LS($LZ), $Heap, n#0 + 1));
        assume CoRecursion.__default.AscendingChainAndRead($LS($LZ), $Heap, n#0)
           == #CoRecursion.Stream.More($Box(n#0), CoRecursion.__default.AscendingChainAndRead($LS($LZ), $Heap, n#0 + 1));
        assume CoRecursion.__default.AscendingChainAndRead#canCall($Heap, n#0 + 1);
        // CheckWellformedWithResult: any expression
        assume $Is(CoRecursion.__default.AscendingChainAndRead($LS($LZ), $Heap, n#0), 
          Tclass.CoRecursion.Stream(TInt));
...
// function declaration for CoRecursion._default.AscendingChainAndPostcondition
function CoRecursion.__default.AscendingChainAndPostcondition($ly: LayerType, n#0: int) : DatatypeType;

function CoRecursion.__default.AscendingChainAndPostcondition#canCall(n#0: int) : bool;

// layer synonym axiom
axiom (forall $ly: LayerType, n#0: int :: 
  { CoRecursion.__default.AscendingChainAndPostcondition($LS($ly), n#0) } 
  CoRecursion.__default.AscendingChainAndPostcondition($LS($ly), n#0)
     == CoRecursion.__default.AscendingChainAndPostcondition($ly, n#0));

// fuel synonym axiom
axiom (forall $ly: LayerType, n#0: int :: 
  { CoRecursion.__default.AscendingChainAndPostcondition(AsFuelBottom($ly), n#0) } 
  CoRecursion.__default.AscendingChainAndPostcondition($ly, n#0)
     == CoRecursion.__default.AscendingChainAndPostcondition($LZ, n#0));

// consequence axiom for CoRecursion.__default.AscendingChainAndPostcondition
axiom 5 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, n#0: int :: 
    { CoRecursion.__default.AscendingChainAndPostcondition($ly, n#0) } 
    CoRecursion.__default.AscendingChainAndPostcondition#canCall(n#0)
         || (5 != $FunctionContextHeight && LitInt(0) <= n#0)
       ==> Lit(false)
         && $Is(CoRecursion.__default.AscendingChainAndPostcondition($ly, n#0), 
          Tclass.CoRecursion.Stream(TInt)));

function CoRecursion.__default.AscendingChainAndPostcondition#requires(LayerType, int) : bool;

// #requires axiom for CoRecursion.__default.AscendingChainAndPostcondition
axiom (forall $ly: LayerType, n#0: int :: 
  { CoRecursion.__default.AscendingChainAndPostcondition#requires($ly, n#0) } 
  LitInt(0) <= n#0
     ==> CoRecursion.__default.AscendingChainAndPostcondition#requires($ly, n#0) == true);

// definition axiom for CoRecursion.__default.AscendingChainAndPostcondition (revealed)
axiom 5 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, n#0: int :: 
    { CoRecursion.__default.AscendingChainAndPostcondition($LS($ly), n#0) } 
    CoRecursion.__default.AscendingChainAndPostcondition#canCall(n#0)
         || (5 != $FunctionContextHeight && LitInt(0) <= n#0)
       ==> CoRecursion.__default.AscendingChainAndPostcondition#canCall(n#0 + 1)
         && CoRecursion.__default.AscendingChainAndPostcondition($LS($ly), n#0)
           == #CoRecursion.Stream.More($Box(n#0), CoRecursion.__default.AscendingChainAndPostcondition($ly, n#0 + 1)));

// definition axiom for CoRecursion.__default.AscendingChainAndPostcondition for all literals (revealed)
axiom 5 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, n#0: int :: 
    {:weight 3} { CoRecursion.__default.AscendingChainAndPostcondition($LS($ly), LitInt(n#0)) } 
    CoRecursion.__default.AscendingChainAndPostcondition#canCall(LitInt(n#0))
         || (5 != $FunctionContextHeight && LitInt(0) <= n#0)
       ==> CoRecursion.__default.AscendingChainAndPostcondition#canCall(LitInt(n#0 + 1))
         && CoRecursion.__default.AscendingChainAndPostcondition($LS($ly), LitInt(n#0))
           == Lit(#CoRecursion.Stream.More($Box(LitInt(n#0)), 
              Lit(CoRecursion.__default.AscendingChainAndPostcondition($LS($ly), LitInt(n#0 + 1))))));

procedure CheckWellformed$$CoRecursion.__default.AscendingChainAndPostcondition(n#0: int where LitInt(0) <= n#0);
...
implementation CheckWellformed$$CoRecursion.__default.AscendingChainAndPostcondition(n#0: int)
{
  var $_Frame: <beta>[ref,Field beta]bool;
  var ##n#0: int;
  var b$reqreads#0: bool;
    ...
    // AddWellformednessCheck for function AscendingChainAndPostcondition
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Corecursion.dfy(20,11): initial state"} true;
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    if (*)
    {
        assume $Is(CoRecursion.__default.AscendingChainAndPostcondition($LS($LZ), n#0), 
          Tclass.CoRecursion.Stream(TInt));
        assume false;
        assume false;
    }
    else
    {
        ...
        assert ##n#0 < n#0;
        assume CoRecursion.__default.AscendingChainAndPostcondition#canCall(n#0 + 1);
        assume CoRecursion.Stream.More_q(CoRecursion.__default.AscendingChainAndPostcondition($LS($LZ), n#0 + 1));
        assume CoRecursion.__default.AscendingChainAndPostcondition($LS($LZ), n#0)
           == #CoRecursion.Stream.More($Box(n#0), 
            CoRecursion.__default.AscendingChainAndPostcondition($LS($LZ), n#0 + 1));
        assume CoRecursion.__default.AscendingChainAndPostcondition#canCall(n#0 + 1);
        // CheckWellformedWithResult: any expression
        assume $Is(CoRecursion.__default.AscendingChainAndPostcondition($LS($LZ), n#0), 
          Tclass.CoRecursion.Stream(TInt));
...
implementation CheckWellformed$$CoRecursion.__default.Prefix(CoRecursion._default.Prefix$_T0: Ty, n#0: int, s#0: DatatypeType)
{
  ...
  var b$reqreads#0: bool;
    ...
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/Corecursion.dfy(28,11): initial state"} true;
    $_Frame := (lambda<alpha> $o: ref, $f: Field alpha :: 
      $o != null && read($Heap, $o, alloc) ==> false);
    if (*)
    {
