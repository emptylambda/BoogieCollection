...
axiom 0 <= $FunctionContextHeight
   ==> (forall x#0: int :: 
    ...
    OpaqueTest.__default.bar#canCall(LitInt(x#0))
         || (0 != $FunctionContextHeight && Lit(x#0 > 7))
       ==> OpaqueTest.__default.bar(LitInt(x#0)) == LitInt(x#0 - 2));

procedure CheckWellformed$$OpaqueTest.__default.bar(x#0: int);
...
axiom 1 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, x#0: int :: 
    ...
    OpaqueTest.__default.foo#canCall(x#0)
         || (1 != $FunctionContextHeight && x#0 > 7)
       ==> true);
...
axiom 1 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, x#0: int :: 
    ...
    OpaqueTest.__default.foo#canCall(LitInt(x#0))
         || (1 != $FunctionContextHeight && Lit(x#0 > 7))
       ==> OpaqueTest.__default.bar#canCall(LitInt(x#0))
         && OpaqueTest.__default.foo($LS($ly), LitInt(x#0))
           == LitInt(OpaqueTest.__default.bar(LitInt(x#0))));

procedure {:autoReq} {:opaque} CheckWellformed$$OpaqueTest.__default.foo(x#0: int);
  free requires 1 == $FunctionContextHeight;
  modifies $Heap, $Tick;



implementation {:autoReq} {:opaque} CheckWellformed$$OpaqueTest.__default.foo(x#0: int)
{
  ...
  var b$reqreads#0: bool;
    ...
    assume {:captureState "/Users/jeffc/Desktop/boogie/Benchmarks/Dafny_ProgramCollection/dafny0/AutoReq.dfy(310,32): initial state"} true;
    ...
    else
    {
        ...
        assert {:subsumption 0} ##x#0 > 7;
        assume ##x#0 > 7;
        b$reqreads#0 := (forall<alpha> $o: ref, $f: Field alpha :: false ==> $_Frame[$o, $f]);
        assume OpaqueTest.__default.bar#canCall(x#0);
        assume OpaqueTest.__default.foo(StartFuel_OpaqueTest._default.foo, x#0)
           == OpaqueTest.__default.bar(x#0);
        assume OpaqueTest.__default.bar#canCall(x#0);
        // CheckWellformedWithResult: any expression
        assume $Is(OpaqueTest.__default.foo(StartFuel_OpaqueTest._default.foo, x#0), TInt);
        assert b$reqreads#0;
    }
}
...
procedure {:auto_generated} {:opaque_reveal} {:verify false} CheckWellformed$$OpaqueTest.__default.reveal__foo();
  free requires 2 == $FunctionContextHeight;
  modifies $Heap, $Tick;



const MoreFuel_OpaqueTest._default.foo0: LayerType;

procedure {:auto_generated} {:opaque_reveal} {:verify false} Call$$OpaqueTest.__default.reveal__foo();
  modifies $Heap, $Tick;
  // frame condition
  free ensures old($Heap) == $Heap;
  free ensures StartFuel_OpaqueTest._default.foo == $LS(MoreFuel_OpaqueTest._default.foo0);
  free ensures StartFuelAssert_OpaqueTest._default.foo
     == $LS($LS(MoreFuel_OpaqueTest._default.foo0));
  // Shortcut to LZ
  free ensures AsFuelBottom(MoreFuel_OpaqueTest._default.foo0)
     == MoreFuel_OpaqueTest._default.foo0;
